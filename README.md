...
هر توسعه‌دهنده می‌تواند همان هفت سند و تصویر کاملاً ساختگی را روی دیتابیس و Storage محلی
خود ایجاد کند. ابتدا نتیجه را بدون هیچ تغییری ببینید:

```powershell
pnpm documents:demo:preview
```

سپس، در حالی که PostgreSQL و Antivirus محلی فعال‌اند، داده‌ها را با یک فرمان اعمال کنید:
