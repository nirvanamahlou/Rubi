...
## 2026-09-22 — PROCUREMENT-QA-FIXES-0922 — PC-B — READY_FOR_REVIEW

- درخواست مالک: ایرادهای قابل‌بازتولید گزارش QA خرید و تأمین رفع و سناریوهای دستی و خودکار دوباره اجرا شوند.
- شاخه مستقل `codex/pc-b-procurement-qa-fixes-0922` از وضعیت جاری Procurement ساخته شد. محدوده: `apps/web/src/modules/procurement/**`، APIهای ماژول Procurement در صورت نیاز، تست‌های هدفمند و ثبت وضعیت همین واحد کار.
- هدف‌ها: اتمیک و مقاوم‌کردن ذخیره/انتشار درخواست و کلیک تکراری، همگام‌سازی فهرست و فیلترها، بازیابی هویت پس از خطای اتصال، پیام‌های اعتبارسنجی دقیق، برچسب‌گذاری و الزامات دسترس‌پذیر، نمایش نام انسانی به‌جای UUID، تمرکز Skip Link و محدودیت طول عنوان.
- بدون Schema/Migration، Dependency/Lockfile، عملیات مالی واقعی، پیام بیرونی یا حذف داده. فایل تولیدی محلی `apps/web/next-env.d.ts` خارج از محدوده و دست‌نخورده می‌ماند.
- نتیجه: ایجاد و انتشار درخواست در یک تراکنش انجام می‌شود، اعتبارسنجی پیش از Mutation و قفل کلیک تکراری مانع پیش‌نویس پنهان و ثبت تکراری است، خطا به عمل «ذخیره» یا «انتشار» و کنترل نامعتبر مربوط می‌شود، فهرست‌های نمونه با فیلتر/تاریخ همگام می‌مانند، نام تأمین‌کننده و برچسب تاریخچه به‌صورت انسانی نمایش داده می‌شود و بازیابی نشست هویت سربرگ را تازه می‌کند. Skip Link، الزامات فیلد و محدودیت طول عنوان نیز اصلاح شدند.
- اعتبارسنجی: ۲۹ تست متمرکز Web و بازاجرای ۹ تست فرم، lint محدوده و typecheck API/Web، build تولیدی API/Web و HTTP 200 مسیرهای Web/API موفق‌اند. آزمون عملی با حساب ساختگی، انتشار اتمیک بدون تأمین‌کننده، جلوگیری از ثبت تکراری، HR dropdown، تقویم، URL/Back، جایگاه منو زیر سرمایه انسانی، RTL و کنسول بدون خطا را تأیید کرد. تست‌های DB-gated به‌دلیل نبود دیتابیس ایزوله اجرا نشدند و viewport موبایل/تبلت به‌دلیل اعمال‌نشدن override مرورگر اجرا‌نشده ثبت شد.
