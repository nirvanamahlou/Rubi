...
## WORKBENCH-034 — PC-B — LOCAL_COMPLETE / RUNTIME_ACTIVE

- Activate the Home feedback/survey form as an authenticated, persisted Workbench workflow. Reserve Workbench feedback Web/API/contracts/tests, the additive `workbench_feedback` Prisma model/migration, the public HR department-recipient lookup, Documents feedback-attachment validation, Notifications delivery integration, and scoped status/data-model docs. Migration Owner = PC-B/WORKBENCH-034 after the completed PC-A supplier-purchase and manifest migrations; no newer active Migration Owner was found on `origin/develop`.
