## RESERVATION-ACTION-DENSITY-0916 — PC-A — READY_FOR_REVIEW

- درخواست مالک در 2026-09-16: پنل عملیات قرارداد زیر جدول بماند، اما به اندازهٔ سابق و فشرده باشد تا جدول قرارداد و عملیات اصلی در یک نما انجام شوند؛ جدول فقط خودش اسکرول داخلی داشته باشد.
- Branch مستقل `codex/pc-a-reservation-action-density-0916` از `origin/develop@38669f42`؛ `COMPUTER_ID=PC-A`.
- محدوده رزرو: فقط CSS پنل عملیات و جدول رزرواسیون، به‌همراه اسناد همین واحد کار. بدون Migration، API، Permission، دادهٔ عملیاتی، Dependency/Lockfile یا Runtime مشترک.
- بررسی مالکیت: واحد قبلیِ همین پنل در `develop` ادغام شده و مالک فعال دیگری برای CSS هدف ثبت نشده است؛ تغییرات رزواسیون در فایل‌های دیگر حفظ می‌شوند.
- نتیجه: پنل پایین جدول با خلاصهٔ قرارداد و سه گروه عملیات در یک ردیف دسکتاپ فشرده شد؛ دکمه‌ها دو ستونه و ۳۲ پیکسلی‌اند. جدول ۳۷۶ پیکسل، نزدیک هفت ردیف، ارتفاع دارد و فقط خودش اسکرول می‌شود.
- اعتبارسنجی: ۲۱ تست هدفمند رزواسیون، lint، typecheck و build تولیدی Web با ۴۶ مسیر موفق‌اند. Migration، دادهٔ عملیاتی و Runtime مشترک تغییر نکردند.
