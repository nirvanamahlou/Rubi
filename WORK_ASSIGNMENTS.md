...
| CUSTOMER-001          | PC-A         | `codex/pc-a-customer-persistence`           | مشتریان، Migration افزایشی، رمزنگاری Contact، Audit redaction و Duplicate query؛ config: `.env.example`، `apps/api/.env.example`، `apps/api/src/config/environment.validation.ts` | `IN_PROGRESS` | اصلاح Review در Draft PR #19؛ Migration موجود immutable؛ بدون Dependency/Lockfile |
