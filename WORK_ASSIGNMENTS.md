...
- نتیجه: متن مبلغ فشرده اکنون بر اساس طول نمایشی خود، از `text-lg` به `text-sm` و برای مقدارهای بسیار بلند به `text-xs` کاهش می‌یابد؛ بنابراین با حفظ نماد کنار عدد، تگ رشد و روند، از عرض کارت خارج نمی‌شود. TypeScript و lint Web موفق‌اند.
