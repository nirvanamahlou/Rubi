...
- نتیجه: حالت اولیهٔ گروه‌های باز در Sidebar اکنون خالی است. زیرصفحه‌ها تنها پس از انتخاب دکمهٔ بازکردن همان گروه نمایش داده می‌شوند و انتخاب صفحهٔ اصلی دیگر هیچ گروهی را خودکار باز نمی‌کند.
...
- نتیجه: `ReportingWorkspace` حالت تعبیه‌شدهٔ صرفاً پیکربندی دارد؛ URL Dashboard تغییر نمی‌کند و فرم موجود، با تمام کنترل‌های فیلتر، اجرا، ذخیره و خروجی، به‌صورت دیالوگ باز می‌شود. برای KPI و نمودارهای دارای گزارش مشخص، دکمه‌ها اکنون همان دیالوگ را باز می‌کنند؛ مورد فاقد گزارش مشخص، صریحاً غیرفعال می‌ماند.
...
- قرارداد نخستِ فهرست فیلترشده به‌صورت پیش‌فرض انتخاب می‌شود تا عملیات بدون انتخاب دستیِ اضافه در دسترس باشد. API، Migration، دادهٔ عملیاتی، Permission، وابستگی و Lockfile در این بهبود تغییر نکرده‌اند.
...
- نتیجه: بخش «فیچرهای استفاده‌شده در فرمول» از `definition.source` هر KPI ساخته شد. نام‌هایی مانند `recognized sale revenue` و `matched purchase cost` همراه با توضیح کوتاهِ منبع واقعی‌شان دیده می‌شوند؛ برای منبع تازه نیز fallback صریح، بدون مقدار یا دادهٔ ساختگی، وجود دارد. فقط Dashboard UI، تست و اسناد تغییر کرده‌اند؛ API، Reports، داده و Migration تغییری ندارند.
- اعتبارسنجی: ۱۶ تست هدفمند Dashboard، TypeScript و lint Web موفق‌اند.

## DASHBOARD-KPI-DEFINITION-PANEL-CLEANUP-0917 — PC-C — LOCAL_COMPLETE

- درخواست مالک: پنل تعریف KPI کوتاه و قابل‌فهم باشد؛ شناسه‌ها، منابع فنی و حاکمیت حذف شوند؛ عنوان شاخص در بالای پنل مرکزچین باشد؛ بستن با ضربدر انجام شود و CTA گزارش رنگ سفید داشته باشد.
- نتیجه: header پنل فقط عنوان KPI و نام فنی را نگه می‌دارد؛ عنوان بزرگ و center است و کنترل Close آیکون X با aria-label دارد. badgeهای KPI ID/role/report، بخش فیچرها و منابع داده و بخش حاکمیت و ردگیری حذف شدند. CTA گزارش `text-white` و رنگ آیکون سفید دارد. API، Reports، داده و Migration تغییر نکرده‌اند.
...
- محدودهٔ رزروشده: رندر Donut، سربرگ Dashboard، پنج asset تصویری فشردهٔ Dashboard، تست Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، Projection، Reports، دیتابیس، Migration، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: دادهٔ نمودار و labelهای واقعی دست‌نخورده می‌مانند؛ تصاویر با Next Image و کیفیت نمایش بهینه فقط برای صفحهٔ فعال بارگذاری می‌شوند و هیچ request خارجی ندارند.
- نتیجه: leader line هر بخش Donut اکنون از محیط حلقه خارج می‌شود، یک مسیر جدا تا ستون ثابت برچسب می‌سازد و عنوان و مقدار/درصد را در دو سطرِ خوانا و دور از حلقه قرار می‌دهد. پنج تصویر دکوراتیوِ بدون متن برای گروه‌های نمای مدیریتی، عملکرد تجاری، مالی و خزانه‌داری، مشتری و رشد و سرمایه انسانی افزوده شد؛ همهٔ زیرصفحه‌های هر گروه همان تصویر مرجع را می‌گیرند. `next/image` با WebP و quality 45 فقط تصویر سربرگِ صفحهٔ فعال را بهینه و بارگذاری می‌کند؛ dependency یا درخواست خارجی اضافه نشده است.
...
- محدودهٔ رزروشده: فقط رندر Donut، تست Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، داده/محاسبه، Reports، دیتابیس، Migration، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: فقط دسته‌های کم‌تعداد Donut می‌مانند؛ label، مقدار، درصد و مجموع از دادهٔ واقعی همان visual می‌آیند و KPI/سود/هزینهٔ ساختگی تولید نمی‌شود.
- نتیجه: تمام Donutهای Dashboard اکنون حلقهٔ بزرگ رتبه‌رنگ دارند؛ نام دسته، مقدار فشرده و درصد واقعی بیرون حلقه با خط راهنما نمایش داده می‌شود و مجموع واقعی همان نمودار در مرکز است. Legend ستونی قبلی حذف شد. قاعدهٔ تبدیل بیش از ۶ دسته به Bar حفظ شده است.
...
- محدودهٔ رزروشده: فقط رفتار چیدمان Sidebar و تست Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، Reports، دیتابیس، Migration، دادهٔ عملیاتی، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: در دسکتاپ، کل Sidebar با هر دو Tab ناوبری و فیلتر sticky است و در ارتفاع محدود viewport اسکرول داخلی دارد؛ موبایل با سند حرکت می‌کند.
- نتیجه: `sticky` از شرط Tab مستقل شد؛ بنابراین در دسکتاپ هم «صفحه‌های داشبورد» و هم «فیلترهای این صفحه» زیر نوار بالایی ثابت می‌مانند. ارتفاع محدود و اسکرول داخلی برای دسترسی به همهٔ کنترل‌های فیلتر حفظ شده است.

## DASHBOARD-NAVIGATION-STICKY-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: پنل «صفحه‌های داشبورد» در Sidebar راست هنگام اسکرول از دید کاربر خارج نشود.
- محدودهٔ رزروشده: فقط رفتار چیدمان Sidebar و تست Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، Reports، دیتابیس، Migration، دادهٔ عملیاتی، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: فقط پنل ناوبری صفحه‌ها در دسکتاپ sticky می‌شود؛ پنل فیلترهای همان صفحه همچنان با سند حرکت می‌کند. فهرست بلند ناوبری اسکرول داخلی و قابل‌دسترسی خواهد داشت.
- نتیجه: پنل «صفحه‌های داشبورد» از breakpoint دسکتاپ با `top-20` ثابت می‌ماند و حداکثر ارتفاع آن متناسب با viewport است؛ در نتیجه با اسکرول صفحه پنهان نمی‌شود و در صورت بلندی فهرست، اسکرول داخلی دارد. پیگیری بعدی مالک همین رفتار را به پنل فیلترها نیز تعمیم داد؛ موبایل همچنان non-sticky است.

## DASHBOARD-ENGLISH-NUMERIC-OUTPUT-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: همهٔ مقدارهای عددی KPI Cardها و نمودارهای Dashboard با ارقام لاتین و واحدهای فشردهٔ انگلیسی `K/M/B` نمایش داده شوند؛ فقط نماد ارز بدون تغییر بماند.
- محدودهٔ رزروشده: فقط قالب‌بندی نمایش و تست Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، دادهٔ source، محاسبه، نرخ ارز، Reports، دیتابیس، Migration، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: مقدار خام و ارز هر سری مستقل حفظ می‌شود؛ این تغییر فقط presentation است و مقدار دقیق در Tooltip/پنل KPI همچنان قابل بازیابی خواهد بود.
- نتیجه: همهٔ KPI Cardها، تگ رشد، Tooltip، نمودارها، Legendها، جدول‌های تحلیلی و خلاصه‌های دسترس‌پذیر Dashboard ارقام `en-US` دارند. خروجی فشردهٔ کارت و محورهای نمودار از `K/M/B` استفاده می‌کند؛ از جمله `﷼ 858.8M` و `$ 44.1M`. نماد ارز و مقدارهای واقعی/سری‌های مستقل ارز بدون تغییر باقی می‌مانند.

## DASHBOARD-SIDEBAR-INFORMATION-ARCHITECTURE-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: عنوان‌های بخش‌های Sidebar راست Dashboard با محتوای هر صفحه و فیلترهای همان صفحه بازبینی و در صورت ابهام، شفاف و استاندارد شوند.
- محدودهٔ رزروشده: فقط ناوبری/فیلترهای Dashboard Web، تست هدفمند و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، Reports، دیتابیس، Migration، دادهٔ عملیاتی، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: شناسهٔ صفحه‌ها، پارامترهای URL و منطق فیلتر ثابت می‌مانند؛ تغییر فقط در Copy و عنوان‌های قابل‌فهم، متناسب با scope واقعی هر فیلتر است.
- نتیجه: نام صفحه‌های Dashboard به دلیل هم‌خوانی با KPI/نمودارهای واقعی حفظ شدند. عنوان عمومی Sidebar به «صفحه‌های داشبورد» و Tab دوم به «فیلترهای این صفحه» تغییر کرد و Heading فیلتر نام صفحهٔ فعال را نشان می‌دهد. نام فیلترهای مشترک شفاف شدند؛ همچنین «وضعیت» در هر صفحه با دامنهٔ واقعی خود (مانند پرداخت، رزرو و صدور، مالی یا پشتیبانی) نمایش می‌یابد. فیلترهای نامرتبط «خدمت سفر» از دو صفحهٔ HR حذف و نام‌های «کانال جذب»، «کارشناس CRM»، «کارشناس فروش» و «کارمند» در صفحه‌های متناظر اعمال شدند.

## DASHBOARD-KPI-COMPACT-VALUE-FIT-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: وقتی مقدار فشردهٔ پولی KPI طولانی است، خروجی از کادر کارت بیرون نزند و فونت مقدار به‌صورت متناسب کوچک شود.
- محدودهٔ رزروشده: فقط کامپوننت و تست هدفمند Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، Reports، دیتابیس، Migration، دادهٔ عملیاتی، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: مبلغ و نماد همچنان یک واحد غیرقابل‌شکستن می‌مانند؛ مبلغ دقیق در Tooltip و پنل تعریف KPI باقی می‌ماند و تگ رشد مستقل از متن مبلغ حفظ می‌شود.
- نتیجه: متن مبلغ فشرده اکنون هم‌زمان بر اساس طول نمایشی و عرض واقعی همان KPI Card با واحد `cqw` اندازه می‌گیرد؛ کارت عریض فونت بزرگ‌تری نگه می‌دارد و کارت کوچک فقط هنگام نیاز کوچک می‌شود. نماد کنار عدد، تگ رشد و روند حفظ می‌شوند و خروجی از عرض کارت خارج نمی‌شود. TypeScript و lint Web موفق‌اند.

## DASHBOARD-CALENDAR-MONEY-FILTER-PANELS-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: مبلغ‌های KPI هر ارز به‌صورت واحد غیرقابل‌شکستن و فشرده با نماد در چپِ نزدیک عدد نمایش داده شوند؛ «این هفته/ماه» بر اساس تقویم واقعی `Asia/Tehran` محاسبه شود؛ و Sidebar غیرچسبان Dashboard در بالای خود دو نمای «فضای کار داشبوردها» و «فیلترهای داشبورد» داشته باشد.
- محدودهٔ رزروشده: Projection و قرارداد محلی Reporting Dashboard، کامپوننت/type/test Dashboard Web و اسناد وضعیت. بدون Migration، تغییر Schema، دادهٔ عملیاتی، Reports، وابستگی یا Lockfile.
- قرارداد: مبلغ دقیق همچنان در Tooltip و پنل تعریف KPI قابل بازیابی است؛ هیچ FX یا جمع میان‌ارزی ایجاد نمی‌شود. بازه‌های تقویمی فقط مرز تهران را می‌سازند و مقایسه همچنان با دورهٔ قبل هم‌طول و هم‌ارز انجام می‌شود. Sidebar `sticky` نمی‌شود و با سند صفحه حرکت می‌کند.
- نتیجه: مبلغ KPI اکنون یک `inline-flex` بدون شکستن است؛ نماد با فاصلهٔ کوتاه در چپِ مقدار می‌آید و مقدار فشردهٔ میلیارد/میلیون/هزار یا B/M/K روی کارت نمایش داده می‌شود، درحالی‌که مقدار کامل در Tooltip و پنل KPI باقی می‌ماند. مرزهای امروز، هفته (شروع شنبه)، ماه، فصل و سال از تقویم فارسی در `Asia/Tehran` می‌آیند. Sidebar دو Tab «فضای کار داشبوردها» و «فیلترهای داشبورد» دارد و فقط پنل منتخب را نشان می‌دهد؛ `sticky` نشده است. typecheck و lint API/Web، build API و آزمون مستقیم مرزهای تقویمی موفق‌اند؛ runner آزمون Vitest در این Worktree ظرف زمان مجاز تکمیل نشد.

## DASHBOARD-KPI-COMPARISON-VISIBILITY-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: با انتخاب بازه‌های «این هفته» یا «این ماه»، تگ رشد/کاهش/ثبات KPI باید قابل مشاهده بماند؛ علت نبود آن تحلیل و رفع شود.
- محدودهٔ رزروشده: کامپوننت و تست هدفمند Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، قرارداد، دیتابیس، Migration، Reports، وابستگی و دادهٔ محلی تغییر نمی‌کنند.
- قرارداد: درصد فقط از comparison دریافت‌شدهٔ دورهٔ قبل هم‌طول نمایش می‌یابد. وقتی comparison وجود ندارد یا درصد آن قابل محاسبه نیست، تگ خنثی «—» با توضیح قابل ممیزی نمایش می‌ماند؛ هیچ درصد یا مقدار ساختگی تولید نمی‌شود.
- نتیجه: شرط رندر از وجود `comparison` یا `trend` جدا شد؛ هر KPI دارای خروجی اکنون تگ وضعیت دارد. وقتی API درصد واقعی می‌دهد، فلش/رنگ/درصد همان داده نمایش می‌یابد؛ وقتی comparison غایب است یا مبنای قبلی صفر است، تگ «—» با Tooltip علت باقی می‌ماند. typecheck، lint و build تولیدی Web موفق‌اند. اجرای Vitest هدفمند در این Worktree ممکن نشد، چون باینری `vitest` در node_modules محلی موجود نیست.

## DASHBOARD-VISUAL-READABILITY-CLEANUP-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: کم‌رنگ‌ترین سطح گرادیان خواناتر شود؛ دو پنل «قرارداد state و دسترسی» و «تصمیم‌های باز و metadata» حذف شوند؛ تگ رشد برای بازهٔ زمانی پایدار بماند و ترتیب مقدارهای پولی حفظ شود.
- محدودهٔ رزروشده: فقط کامپوننت/test Dashboard و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، قرارداد، دیتابیس، Migration، Reports، وابستگی و دادهٔ محلی تغییر نمی‌کنند.
- قرارداد: کم‌رنگ‌ترین رنگ همچنان همان رتبهٔ کمترین مقدار را نمایندگی می‌کند، اما حداقل کنتراست قابل‌خواندن دارد. رشد هر ارز از مقایسهٔ مستقل خودش می‌آید و مقایسهٔ تک‌ارزی سازگار نیز fallback می‌شود؛ FX یا دادهٔ ساختگی ممنوع است.
- نتیجه: کم‌رنگ‌ترین رنگ رتبه از `#dbeafe` به `#93c5fd` ارتقا یافت. دو پنل انتهایی درخواست‌شده حذف شدند. تگ رشد در سطر مقدار KPI از comparison سری ارز یا comparison تک‌ارزی می‌خواند؛ اگر دادهٔ دورهٔ قبل وجود نداشته باشد، تگ خنثی «—» با Tooltip روشن باقی می‌ماند. تست Dashboard (۱۶)، lint و TypeScript Web موفق‌اند.

## DASHBOARD-KPI-VALUE-ROW-ALIGNMENT-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: نماد تمام ارزها در سمت چپ مقدار KPI و تگ رشد/کاهش/ثبات در سمت راست همان مقدار عددی قرار گیرد.
- محدودهٔ رزروشده: فقط کامپوننت KPI Dashboard، تست هدفمند و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، قرارداد، دیتابیس، Migration، Reports، وابستگی و دادهٔ محلی تغییر نمی‌کنند.
- قرارداد: مقدار و نماد ارز با `bdi` مجزا و قابل‌خواندن حفظ می‌شوند؛ رشد همچنان از همان ارز و همان دورهٔ قبل می‌آید و جایگزین عدد یا ادغام FX نمی‌شود.
- نتیجه: هر سطر ارزی به شبکهٔ RTL سه‌ستونه تغییر کرد: تگ رشد در راست، مقدار در میانه و نماد ارز در چپ. KPIهای غیرارزی نیز تگ رشد را در سمت راست خروجی می‌گیرند. تست Dashboard (۱۶)، lint و TypeScript Web موفق‌اند.

## DASHBOARD-RANKED-CHARTS-COMPARISON-BADGES-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: رنگ Barهای مقایسه‌ای و Donutها از مقدار بیشتر به کمتر، طیف هم‌خانوادهٔ تیره‌به‌روشن داشته باشد؛ تگ رشد KPI که در کارت‌های پولی چندارزی پنهان مانده، برای هر ارز به‌صورت مستقل نمایش داده شود.
- محدودهٔ رزروشده: قرارداد Projection Dashboard Reporting، service و تست هدفمند API، client/type و کامپوننت/test Dashboard Web و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. Migration، Schema، دادهٔ محلی، Reports، وابستگی و Lockfile تغییر نمی‌کنند.
- قرارداد: درصد تغییر فقط از همان ارز در دورهٔ قبل هم‌طول محاسبه می‌شود. هیچ تبدیل FX یا جمع میان‌ارزی انجام نمی‌شود. طیف رنگی رتبه را نشان می‌دهد و هر رنگ در legend و دادهٔ متنی همان دسته یکسان می‌ماند.
- نتیجه: Bar و Donut با مقیاس آبی تیره‌به‌روشن بر پایهٔ مقدار رتبه‌بندی شدند. API برای KPI پولی `comparisonSeries` هر ارز را می‌فرستد و کارت همهٔ تگ‌های رشد متناظر را روبه‌روی عنوان نمایش می‌دهد. تست Dashboard API (۵)، Dashboard Web (۱۶)، lint و TypeScript هر دو برنامه موفق‌اند.

## DASHBOARD-KPI-TREND-TREATMENT-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک: متن توضیحی تبدیل‌نشدن ارز از KPI Cardها حذف شود؛ نمودار روند پایین کارت‌ها بزرگ‌تر و دارای سطح سایه‌دار با رنگ هر سری باشد؛ نشان رشد/کاهش/ثبات با فلش یا خط تیره و درصد، روبه‌روی عنوان در سربرگ کارت قرار گیرد.
- محدودهٔ رزروشده: فقط `apps/web/src/modules/dashboard/components/dashboard-workspace.tsx`، تست Dashboard و اسناد وضعیت در Worktree `codex/pc-c-dashboard-reporting-latest`. API، قرارداد، دیتابیس، Migration، Reports، وابستگی و دادهٔ محلی تغییر نمی‌کنند.
- قرارداد: هر ارز همچنان سری جدا و بدون FX دارد؛ متن جایگزینِ گمراه‌کننده یا دادهٔ ساختگی اضافه نمی‌شود. توضیح دسترس‌پذیر مقیاس مستقل ارزها در `aria-label` روند حفظ می‌شود.
- نتیجه: Sparkline در پایین کارت به ارتفاع ۵۶px رسید؛ برای هر سری ارز، سطح گرادیانی با رنگ همسان خط و legend افزوده شد. نشان رشد به سمت مقابل عنوان در سربرگ منتقل شد و فقط فلش/خط ثبات به‌همراه درصد نمایش می‌دهد. متن درخواستی حذف شد. تست Dashboard (۱۶)، lint و TypeScript Web موفق‌اند.
...
- محدوده: فقط کامپوننت و تست ماژول Dashboard در Worktree یکپارچهٔ `codex/pc-c-dashboard-reporting-latest`؛ API contract، دیتابیس، Schema/Migration، وابستگی و Reports تغییر نمی‌کنند.
- نتیجه: Bar رتبه‌بندی افقی، Combo ستون/میانگین روند، Donut دارای مرکز و Legend عددی، جدول تحلیلی متراکم و کارت‌های هم‌ارتفاع اجرا شدند. lint، TypeScript، ۱۴۶۷ تست Web و build تولیدی ۴۶ مسیر موفق‌اند؛ Web و API روی ۳۰۰۰ و ۴۰۰۰ فعال‌اند. پذیرش تصویری صفحهٔ محافظت‌شده پس از ورود کاربر انجام می‌شود.

## REPORTING-OPERATIONS-RECENT-30-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک 2026-09-16: سکشن‌های «اجراها» و «خروجی‌ها» فقط ۳۰ رکورد اخیر را نمایش دهند.
- محدوده: محدودسازی queryهای فهرست‌خوانی Reports و guard نمایشی Frontend؛ رکوردهای قدیمی حذف فیزیکی نمی‌شوند و برای ممیزی در دیتابیس باقی می‌مانند.
- نتیجه: API برای هر دو فهرست `runs` و `exports` با `ORDER BY createdAt DESC LIMIT 30` پاسخ می‌دهد و Frontend نیز سقف ۳۰تایی را enforce می‌کند. تست Reports Web (۲ مورد)، lint و TypeScript هر دو برنامه موفق‌اند.

## REPORTING-PIE-CENTER-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک 2026-09-16: نمودار دایره‌ای در بالای کارت و در مرکز افقی سکشن خودش قرار گیرد؛ Legend زیر نمودار باقی بماند.
- محدوده: فقط چیدمان نمودار دایره‌ای و تست هدفمند Reports Web؛ بدون تغییر API، دیتابیس، Dashboard یا منطق محاسبه.
- نتیجه: کانتینر نمودار با Flex عمودی و `items-center` مرکزگرا شد؛ تست Reports Web (۱۶ مورد)، lint و TypeScript موفق‌اند و صفحهٔ Login روی پورت ۳۰۰۰ پاسخ ۲۰۰ می‌دهد.

## REPORTING-PIE-LEGEND-LAYOUT-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک 2026-09-16: در نمای نمودار دایره‌ای فرم پیکربندی Reports، نمودار در بالا و راهنمای رنگی آن به‌صورت جداگانه زیر نمودار باشد تا Legend با نمودار هم‌پوشانی نداشته باشد.
- محدوده: فقط چیدمان و تست هدفمند Workspace گزارش؛ بدون تغییر API، دیتابیس، Dashboard، دادهٔ نمونه یا منطق محاسبه نمودار.
- نتیجه: هر نمودار و کد ارز در بالای کارت است؛ Legend با جداکنندهٔ دیداری و عنوان مشخص زیر آن قرار دارد. تست هدفمند Reports Web (۱۶ مورد)، lint و TypeScript موفق‌اند.

## REPORTING-XLSX-SECTION-SPACING-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک 2026-09-16: در خروجی XLSX گزارش‌ها میان بلوک «نام گزارش و زمان»، جدول فیلترها و جدول داده‌ها هر بار یک سطر خالی قرار گیرد.
- محدوده: فقط مولد XLSX گزارش‌ها و تست هدفمند آن؛ CSV/PDF، API contract، دیتابیس و سایر خروجی‌ها بدون تغییر می‌مانند.
- نتیجه: سطرهای ۲ و ۵ خالی‌اند، جدول داده از سطر ۶ آغاز می‌شود و Freeze Pane و محدودهٔ Excel Table با چیدمان جدید هماهنگ شدند. ۲ تست هدفمند، lint و TypeScript API موفق‌اند.

## REPORTING-RESULT-SCROLL-PAGING-0916 — PC-C — LOCAL_COMPLETE

- درخواست مالک 2026-09-16: نمودارهای نمای نتیجه در فرم پیکربندی گزارش داخل ناحیهٔ خود قابل اسکرول باشند و صفحه‌بندی پنج‌تایی فقط برای جدول اعمال شود.
- محدوده: کامپوننت و تست هدفمند Workspace گزارش در شاخهٔ یکپارچهٔ `codex/pc-c-dashboard-reporting-latest`؛ بدون تغییر API، دیتابیس، Dashboard، Navigation یا داده‌های نمونه.
- تغییرات محلی موجود Dashboard/Reports حفظ می‌شوند و هیچ reset، checkout یا stash انجام نمی‌شود.
- نتیجه: جدول با اندازه صفحهٔ ثابت ۵ و کنترل قبلی/بعدی اجرا می‌شود؛ نمودار تا سقف ۱۰۰ رکورد را یک‌جا دریافت و در ناحیهٔ قابل اسکرول نمایش می‌دهد. ۱۶ تست هدفمند، lint و TypeScript Web موفق‌اند.

## REPORTING-XLSX-LAYOUT-0915 — PC-C — LOCAL_COMPLETE

- درخواست مالک: چیدمان خروجی Excel گزارش‌ها با نام و زمان UTC در سطر اول، فیلترهای دو‌سطره و جدول داده از سطر چهارم؛ حذف کد گزارش و نام Projection از workbook و نمایش هزارگان بدون تغییر شناسه‌ها.
- محدوده: generator و Projection خروجی Reports API، تست‌های همان مسیر و مستندات. شاخهٔ اجرایی مالک `codex/pc-c-dashboard-reporting-latest` و داده‌های محلی/تغییرات دیگر حفظ شدند؛ ارسال به GitHub در این واحد انجام نمی‌شود.
- خروجی، همهٔ ردیف‌های مجازِ بازهٔ گزارش را شامل می‌شود و محدودیت صفحه‌بندی Preview را به فایل تحمیل نمی‌کند. مقدارهای عددیِ قابل‌نمایش دقیق در Excel عددی باقی می‌مانند؛ مبلغ طولانی‌تر از دقت ۱۵رقمی به متن گروه‌بندی‌شده تبدیل می‌شود.

## REPORTING-CASH-POSITION-0915 — PC-C — READY_FOR_REVIEW / PRODUCER_BLOCKED

- درخواست 2026-09-15: بررسی امکان‌سنجی گزارش موقعیت نقدینگی و شاخص‌های تزریق/برداشت برای مدیرعامل؛ تنها منبع مالی تأییدشده مجاز به نمایش عدد است.
...
- محدوده رزرو: Query Service و endpoint نسخه‌دار ارتباطات در `apps/api/src/b2b/**`، قرارداد افزایشی B2B در `packages/contracts/src/b2b/**` و مصرف‌کننده Web Organizations و تست‌هایشان، به‌همراه همین Work Item و سند Task. Backend تطبیق خواندنی Customers → Sales → Reservations را فقط از سرویس عمومی مالک و با Permission/Branch سرور انجام می‌دهد؛ Web هیچ API مالک دیگری را برای این نما مستقیماً تجمیع نمی‌کند.
