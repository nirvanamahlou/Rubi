## RESERVATION-FORM-CHILD-REDESIGN-0916 — PC-A — READY_FOR_REVIEW

- درخواست مالک در 2026-09-16: ردهٔ دقیق کودک هتل در خروجی فرم رزواسیونِ ارسال‌شده به کارگزار، کنار `CHD` نمایش داده شود و پنجرهٔ ویرایش قرارداد با انتخاب‌گرهای خواناتر بازطراحی شود.
- Branch مستقل `codex/pc-a-reservation-form-child-redesign-0916` از `origin/develop@20fc2d20`؛ `COMPUTER_ID=PC-A`.
- محدودهٔ رزرو: مدل و تست فرم رزواسیون، نمایش PDF/پیش‌نمایش همان فرم، ویرایشگر قرارداد رزواسیون، کنترل‌های انتخابی تنظیمات رزواسیون و CSS/testهای نزدیک، به‌همراه اسناد این واحد. هیچ Migration، Seed، دادهٔ عملیاتی، Permission، API عمومی، Dependency/Lockfile یا Runtime مشترک تغییر نمی‌کند.
- ردهٔ بلیط همچنان فقط `CHD` است؛ بازهٔ هتل از snapshot نسخه‌دار فرم رزواسیون خوانده و به شکل پرانتزی `CHD (2-6)` یا `CHD (6-12)` چاپ می‌شود. هیچ ستون جدیدی به جدول فرم اضافه نمی‌شود.
- نتیجه: دادهٔ خروجی فرم رزواسیون و واچر اکنون ردهٔ هتل کودک را به CHD (2-6) یا CHD (6-12) تبدیل می‌کند و شمارندهٔ کودک درست می‌ماند. پنجرهٔ ویرایش هدر، تب‌های شماره‌دار و بخش فعال روشن دارد؛ انتخاب‌گرهای ردهٔ بلیط، ردهٔ کودک هتل و جنسیت با کنترل مشترک و گزینه‌های واضح جایگزین <select>های خام شدند.
- اعتبارسنجی: ۵ تست هدفمند مدل فرم/واچر، lint فایل‌های متاثر، typecheck کامل Web و build تولیدی Web موفق‌اند. Migration، Seed، دادهٔ عملیاتی، API عمومی، Dependency/Lockfile و Runtime مشترک تغییر نکردند.
