...
- نتیجه و اعتبارسنجی: ثبت پیش‌نویسِ بدون مسئول دیگر به projection وظایف دسترسی ندارد؛ بنابراین نبود موقت جدول آن projection تراکنش خرید را rollback نمی‌کند. دکمهٔ «تأیید و انتشار» درخواست کامل را بدون جعل تأییدکننده به `SUBMITTED` می‌برد و «ارسال برای تأیید» از همان وضعیت، سیاست واقعی را اعمال می‌کند. تست متمرکز Tasks و تست یکپارچهٔ guardشدهٔ انتشار افزوده شد؛ lint و typecheck API/Web و build API موفق‌اند. Migration/Schema و دادهٔ عملیاتی تغییر نکردند.
