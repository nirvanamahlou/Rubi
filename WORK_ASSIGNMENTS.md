...
## MARKETING-PROCESS-TRACKING-0923 — PC-B — READY_FOR_REVIEW

- درخواست مالک در 2026-09-23: فرایند Visio بازاریابی از استراتژی و جذب سرنخ تا CRM، فروش، اجرای سفر، وفاداری و تحلیل عملکرد در پروژه قابل پیگیری شود؛ کمبود زیرساخت به‌صورت جایگاه fail-closed باقی بماند و در پایان گزارش شود.
- شاخه مستقل `codex/pc-b-marketing-process-tracking` از `origin/develop@0accfb7a`؛ `COMPUTER_ID=PC-B`.
- محدوده رزروشده: `apps/api/src/marketing/**`، `apps/web/src/modules/marketing/**`، route موجود Marketing، تست‌های همین ماژول، ثبت MarketingModule در `apps/api/src/app.module.ts` و سند Task. اتصال‌های بین‌ماژولی فقط از Public Service/Port موجود مصرف می‌شوند؛ جدول یا Repository خصوصی Customers، Customer Affairs، Sales، Reservations، Reporting و Notifications مستقیماً خوانده نمی‌شود.
- قرارداد افزایشی مجوزها و projection فرایند Marketing در `packages/contracts/src/iam/index.ts`، `packages/contracts/src/marketing/**` و root export، همراه داده Seed مجوز/نقش در `packages/database/src/permission-seed-data.ts` و `packages/database/prisma/seed.ts` فقط برای همین واحد رزرو شد؛ نسخه قبلی و Roleهای موجود سازگار می‌مانند. این واحد Prisma Schema/Migration، Dependency/Lockfile یا مجوزدهی خودکار به Roleهای غیر Marketing را رزرو نمی‌کند. Migration ثبت‌شده `PC-A/TICKET-STANDALONE-SALE-PRICE-0921` با وجود حضور commit آن در `develop` هنوز در همین سند صریحاً آزاد نشده است؛ بنابراین Persistence مارکتینگ تا Handoff رسمی به‌صورت Port و وضعیت `INFRASTRUCTURE_PENDING` باقی می‌ماند و Fake Repository یا داده عملیاتی ساختگی ایجاد نمی‌شود.
- معیار فنی: Modular Monolith/PostgreSQL، PII فقط نزد Customers، API خواندنی p95 زیر ۵۰۰ms و fail-closed، UI داخلی desktop-first/RTL/WCAG AA با LCP≤2.5s، INP≤200ms و CLS≤0.1. قفل اسناد مرکزی فقط برای ثبت همین Work Item و گزارش پایان نزد PC-B است.
- نتیجه: قرارداد `marketing.process.v1`، endpoint مجوزدار `GET /marketing/process`، Seed مجوز/Role و سکشن RTL «فرایند یکپارچه» با هشت مرحله تحویل شد. مرحله‌های عملیاتی به مسیر واقعی ماژول مالک می‌روند و جایگاه‌های فاقد قرارداد/Persistence غیرفعال و صریح باقی مانده‌اند. ۳۹ تست API و ۲۳ تست Web، lint محدوده، typecheck کامل Contracts/Database/API/Web و Build تولیدی API/Web با ۵۲ مسیر موفق‌اند. هیچ Migration، Seed عملیاتی یا اتصال دیتابیس اجرا نشد؛ فایل‌های اجرایی و قفل محدود اسناد این واحد با Commit تحویل `RELEASED` می‌شوند.
