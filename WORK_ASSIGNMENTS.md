...
- Handoff: اتصال کامل همه کارت‌های UI در PR #304 به این قرارداد بعد از Review/ادغام Backend انجام شود. Probe عمومی Redis/Worker/Storage/Queue و Port عمومی Retry Job هنوز از ماژول‌های مالک منتشر نشده‌اند؛ بنابراین وضعیت آن‌ها صادقانه `UNKNOWN` است و Retry ساختگی اضافه نشده است. قفل‌های Migration، قرارداد و اسناد مرکزی با ارسال Draft PR این Slice آزاد می‌شوند.
