...
## MANIFEST-TEMPLATE-PER-TICKET-0915 — PC-A — READY_FOR_REVIEW

- درخواست مالک در 2026-09-15: در تعریف هر بلیط قالب MANIFEST از قالب‌های موجود انتخاب شود؛ در صفحه MANIFEST بلیط‌های بازه با قالب ثبت‌شده و خروجی سادهٔ جایگزین نمایش/دانلود شوند. قالب‌های ازمیر و اسپارتا در اطلاعات پایه قابل مدیریت باشند؛ حذف قالب متصل به بلیط به غیرفعال‌سازی برای بلیط‌های جدید تبدیل و نسخهٔ استفاده‌شده حفظ شود.
- COMPUTER_ID=PC-A؛ شاخهٔ مستقل `codex/pc-a-manifest-template-per-ticket-0915` از `origin/develop@ff15c7d3`. محدوده: Ticket Catalog published offer/فرم و قرارداد Travel، Master Data public template selection/status، Reservations MANIFEST exporter/UI، Migration افزایشی در صورت نیاز، فایل‌های قالب پاک‌سازی‌شده و تست/اسناد همین واحد. هیچ دادهٔ واقعی مسافر، Secret، تغییر IAM یا Lockfile وارد Git نمی‌شود.
- Master Data مالک وضعیت و نسخهٔ قالب، Documents مالک فایل و Ticket Catalog مالک انتخاب قالب بلیط است. Reservations فقط API عمومی هر مالک را مصرف می‌کند. نسخهٔ مرتبط با بلیط پس از غیرفعال‌سازی نیز برای خروجی‌های قبلی باقی می‌ماند.
- تحویل: انتخاب قالب فعال در فرم انتشار بلیط Tour Workspace، snapshot روی بلیط منتشرشده، خروجی بر اساس قالب انتخابی یا Excel ساده، دو فایل ازمیر/اسپارتا پاک‌سازی‌شده در اطلاعات پایه و غیرفعال‌سازی قالب متصل به بلیط. Migration افزایشی آماده است و هنوز روی DB عملیاتی اعمال نشده. Commit `03676bf9` به شاخهٔ مستقل Push شد و PR #287 بدون تعارض با هشت بررسی CI موفق برای develop باز است؛ تا تأیید ادغام نشده است.
