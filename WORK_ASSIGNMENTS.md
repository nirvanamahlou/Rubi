...
## RESERVATION-TICKET-DELIVERY-GATE-0912 — PC-A — LOCAL_COMPLETE

- درخواست مالک محصول: بلیط از لحظه ورود قرارداد به رزرواسیون صادرشده محسوب و در رزرواسیون قابل دانلود PDF باشد؛ فروش و MANIFEST فقط پس از تأیید تحویل مدارک توسط مالی مجاز باشند. محدوده: نمایش/دانلود بلیط، مسیر مجوز فروش، Gate سروری MANIFEST، تست‌ها و اسناد. متن‌های Draft و توضیحی اضافی از خروجی صادرشده حذف می‌شوند. بدون Migration، تغییر IAM، داده واقعی یا push.
- خروجی رزرواسیون با وضعیت `ISSUED` و بدون متن‌های Draft/Notice ساخته می‌شود. دانلود فروش از endpoint مالی خودش عبور می‌کند و MANIFEST پیش از خواندن اطلاعات مسافر Gate مالی را کنترل می‌کند. ۱۳ تست Web، ۸ تست API، lint، typecheck، build تولیدی API/Web و بازبینی تصویری PDF موفق‌اند؛ هر دو سرویس لوکال فعال‌اند.

## OLD-CONTRACT-TRAVEL-QA-0912 — PC-A — LOCAL_COMPLETE

- به درخواست مالک محصول، اطلاعات سفر خارجی پنج قرارداد قدیمی `SC-2026-000001` تا `SC-2026-000005` فقط در پایگاه دادهٔ لوکال با دادهٔ ساختگی تکمیل شد. چهار Customer یکتا تغییر کرد؛ فقط فیلدهای خالی پر، نسخه افزایش و Audit ثبت شد. شماره پاسپورت‌های تستی تازه با سازوکار رمزنگاری Customers ذخیره شدند. بدون تغییر کد، Migration، مجوز، داده واقعی یا push.
