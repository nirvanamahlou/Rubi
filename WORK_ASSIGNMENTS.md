## SYSTEM-MANAGEMENT-001 — PC-B — UI_INTEGRATION_IN_PROGRESS

- مجوز صریح مالک در 2026-09-16: `PC-B` برای تکمیل Backend مدیریت سامانه و ایجاد قراردادهای عمومی لازم در چهار Work Item مستقل مجاز است: `SYSTEM-SETTINGS-API-001` (تنظیمات نسخه‌دار، شماره‌گذاری و تاریخچه)، `SYSTEM-IAM-SESSIONS-001` (نشست‌ها، ابطال نشست و مجوزهای مرتبط)، `SYSTEM-OPS-HEALTH-001` (وضعیت PostgreSQL/Redis/Worker/Storage و Jobها) و `SYSTEM-GOVERNANCE-001` (Feature Flag، درخواست Backup، maker-checker و Audit حساس).
- سطح دسترسی اعطاشده: `PC-B` در Work Itemهای بالا مجاز به تغییر کد مالک IAM/Settings/System، قراردادهای عمومی، permission catalog و grantهای حداقلی همان قابلیت، Schema و Migration افزایشی، Job/Worker، Seed تستی و تنظیمات غیرمحرمانه است. اجرای Migration، ایجاد/ویرایش/حذف دادهٔ مصنوعی، ابطال نشست مصنوعی و rehearsal بازیابی Backup در دیتابیس ایزولهٔ local/test نیز مجاز است. Secretها فقط از reference استاندارد محیط مصرف می‌شوند و مقدارشان خوانده، ثبت یا منتشر نمی‌شود.
- مرز دسترسی عملیاتی: این واگذاری به‌تنهایی credential یا نقش Production ایجاد نمی‌کند. اجرای Migration، Backup/Restore، حذف داده، ابطال نشست واقعی، تغییر Feature Flag یا Grant روی Production فقط با تأیید موردی همان اجرا، هویت مجاز محیط و ثبت Audit انجام می‌شود. Push مستقیم به `main`/`develop`، دورزدن Review و تغییر محدودهٔ دارای lock فعال همچنان مجاز نیست.
- حدود این مجوز: هر Work Item باید پیش از تغییر کد، محدودهٔ فایل و قرارداد producer/consumer خود را جدا رزرو کند و در Branch/PR مستقل اجرا شود. تغییر additive در Schema/Migration فقط پس از اثبات نیاز و ثبت `Migration Owner` اختصاصی مجاز است؛ دسترسی مستقیم میان جدول‌های ماژول‌ها، افشای Secret/Token/PII، Grant گسترده، اجرای Backup/Restore واقعی، حذف داده یا ابطال نشست عملیاتی خارج از محیط کنترل‌شده همچنان ممنوع است. در صورت مشاهدهٔ lock یا مالک فعال هم‌پوشان، کار همان محدوده متوقف و هماهنگ می‌شود.
- Backend مدیریت سامانه در PR #305 بازبینی و در `develop` ادغام شد. PR #304 اکنون فقط مصرف‌کنندهٔ UI این قراردادهای نسخه‌دار است؛ تغییر backend یا Migration تازه‌ای در این PR انجام نمی‌شود.
...
- محدودهٔ رزروشده: صفحه و تست‌های `/system`، client نسخه‌دار مدیریت سامانه و مستندات همین واحد. UI فقط APIهای عمومی IAM، Legal Entity و `system-management/v1` را مصرف می‌کند؛ مالکیت داده یا قرارداد هیچ‌کدام جابه‌جا نمی‌شود.
- مرز قطعی: بدون Prisma schema/Migration/Seed، Dependency/Lockfile، Grant واقعی یا دسترسی مستقیم به جدول ماژول دیگر. برای Redis، Worker و Queue که هنوز adapter/Port عمومیِ مالک ندارند، UI فقط وضعیت `UNKNOWN` واقعی را نمایش می‌دهد و موفقیت یا اتصال ساختگی ندارد.
