...
- نتیجه: پنل «صفحه‌های داشبورد» از breakpoint دسکتاپ با `top-20` ثابت می‌ماند و حداکثر ارتفاع آن متناسب با viewport است؛ در نتیجه با اسکرول صفحه پنهان نمی‌شود و در صورت بلندی فهرست، اسکرول داخلی دارد. پنل فیلترها و موبایل همچنان non-sticky هستند.
