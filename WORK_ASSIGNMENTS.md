...
- نتیجه: کارت‌ها و فرم‌های عملیاتی `/system` به تنظیمات نسخه‌دار، شماره‌گذاری، نشست‌ها، Feature Flag، درخواست Backup، Health، Audit و Retry واقعی Reporting Export وصل‌اند. Probe Storage از Public Port مالک Documents و PostgreSQL از query فقط‌خواندنی استفاده می‌کند. Redis، Worker و Queue تا انتشار adapter/Port مالک، صادقانه `UNKNOWN` می‌مانند؛ هیچ اتصال یا موفقیت ساختگی ثبت نشده است.
