...
export function createdDateFilter(query: { createdFrom?: string; createdBefore?: string }) {
  const parse = (value: string) => {
    const date = new Date(value);
    if (!/(?:Z|[+-]\d{2}:\d{2})$/.test(value) || !Number.isFinite(date.getTime()))
      throw new BadRequestException('تاریخ فیلتر باید زمان معتبر با منطقه زمانی باشد.');
    return date;
  };
