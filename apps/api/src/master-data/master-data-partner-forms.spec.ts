...
function setup(
  contact = { id: contactId, organizationId: orgId, isActive: true },
) {
  const organization = {
    id: orgId,
    isActive: true,
  ...
  const repository = {
    ...
    list: vi.fn(async () => ({
      rows: [{ id: contactId, code: 'HOTEL', isActive: true }],
      total: 1,
    })),
...
describe('partner profile form persistence', () => {
  ...
  it('does not serialize primary-contact private data or inactive masks', () => {
    const source = {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      organization: { personType: 'LEGAL' },
      primaryContact: {
        isActive: true,
        fullName: 'Test contact',
        phoneMasked: '••••1234',
        emailMasked: null,
        phoneEncrypted: 'never-public',
        phoneFingerprint: 'never-public',
      },
    };
    expect(toMasterDataRecord('brokers', source).attributes).toMatchObject({
      primaryPhoneMasked: '••••1234',
      organizationPersonType: 'LEGAL',
    });
    expect(JSON.stringify(toMasterDataRecord('brokers', source))).not.toContain(
      'never-public',
    );
    expect(
      toMasterDataRecord('brokers', {
        ...source,
