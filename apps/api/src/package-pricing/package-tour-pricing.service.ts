...
export class PackageTourPricingService {
  ...
  /** Published tour prices are read by Sales, without using the Ticket Catalog list fare. */
  async salesChoices(
    tourId: string,
    actor: AuthenticatedActor,
  ): Promise<readonly PackageTourSalesPriceChoiceV1[]> {
    const grid = await this.pricing.tourCostGrid(tourId, actor);
    const drafts = await this.database.client.packagePricingTourDraft.findMany({
      where: { tourDepartureId: tourId, branchId: grid.tour.branchId },
      select: { id: true },
    });
    ...
    const rows =
      await this.database.client.packagePricingTourPublishedVersion.findMany({
        ...
        include: { draft: true, roomPrices: true },
      });
    const latest = new Set<string>();
    return rows.flatMap((row): PackageTourSalesPriceChoiceV1[] => {
      if (latest.has(row.draftId)) return [];
      latest.add(row.draftId);
      const batch = grid.purchaseBatches.find(
        (item) => item.id === row.draft.batchId,
      );
      if (!batch) return [];
      return row.roomPrices.flatMap(
        (price): PackageTourSalesPriceChoiceV1[] => {
          const rate = batch.rows.find((item) => item.id === price.hotelRateId);
          if (!rate) return [];
          return [
            {
              ...
              hotelSale: price.hotelSale.toString(),
