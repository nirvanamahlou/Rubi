...
export class ReportSortDto {
  ...
  @ApiProperty({ enum: ['ASC', 'DESC'] }) @IsIn(['ASC', 'DESC']) direction!:
    'ASC' | 'DESC';
}

export class ReportQueryDto {
  @ApiProperty({ type: Object }) @IsObject() filters!: Record<
    string,
    string | string[]
  >;
...
export class SaveReportDto {
  ...
  @IsEnum({ PERSONAL: 'PERSONAL', TEAM: 'TEAM' })
  sharingScope!: 'PERSONAL' | 'TEAM';
  @ApiPropertyOptional() @IsOptional() @IsBoolean() isFavorite?: boolean;
  @ApiProperty({ type: Object }) @IsObject() filterState!: Record<
    string,
    unknown
  >;
  @ApiPropertyOptional({
...
export class ShareSavedReportDto {
  @ApiProperty({
    type: [String],
    description: 'شناسه کاربران دریافت‌کننده گزارش',
  })
  @IsArray()
  @ArrayMaxSize(50)
  @IsUUID('4', { each: true })
...
export class ExportCreateDto {
  ...
  @IsIn(['CSV', 'XLSX', 'PDF'])
  format!: 'CSV' | 'XLSX' | 'PDF';
  @ApiProperty({ type: ReportQueryDto })
  @ValidateNested()
  @Type(() => ReportQueryDto)
