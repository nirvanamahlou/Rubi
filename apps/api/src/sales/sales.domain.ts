...
function decimalUnits(value: string): bigint {
  if (!DECIMAL.test(value))
    throw new SalesDomainError('SALES_MONEY_INVALID', 'مبلغ Decimal معتبر نیست.');
...
export function sumSalesDecimals(values: readonly string[]): string {
  return unitsDecimal(values.reduce((sum, value) => sum + decimalUnits(value), 0n));
}

function currency(value: string): string {
  const normalized = value.trim().toUpperCase();
  if (!/^[A-Z]{3}$/.test(normalized))
    throw new SalesDomainError('SALES_CURRENCY_INVALID', 'کد ارز باید ISO سه‌حرفی باشد.');
  return normalized;
}
...
export function passengerAgeCategory(
  ...
  if (birth > departure || Number.isNaN(birth.getTime()))
    throw new SalesDomainError('SALES_PASSENGER_BIRTH_DATE_INVALID', 'تاریخ تولد مسافر معتبر نیست.');
  let age = departure.getUTCFullYear() - birth.getUTCFullYear();
  const beforeBirthday =
    departure.getUTCMonth() < birth.getUTCMonth() ||
    (departure.getUTCMonth() === birth.getUTCMonth() && departure.getUTCDate() < birth.getUTCDate());
...
export function validateSalesContract(input: SalesContractCreateRequest): void {
  for (const [label, value] of [
    ...
    if (!UUID.test(value))
      throw new SalesDomainError('SALES_REFERENCE_INVALID', `شناسه ${label} معتبر نیست.`);
  if (input.originId === input.destinationId)
    throw new SalesDomainError('SALES_ROUTE_INVALID', 'مبدأ و مقصد نمی‌توانند یکسان باشند.');
  const departure = validDate(input.departureDate, 'تاریخ رفت');
  if (input.tripType === 'ROUND_TRIP' && !input.returnNotBefore)
    throw new SalesDomainError('RETURN_TICKET_INVALID', 'برای سفر رفت‌وبرگشت تاریخ بازگشت لازم است.');
  if (input.returnNotBefore && validDate(input.returnNotBefore, 'تاریخ بازگشت') < departure)
    throw new SalesDomainError('RETURN_TICKET_INVALID', 'تاریخ بازگشت نباید قبل از رفت باشد.');
  if (!input.services.length || !input.passengers.length || !input.priceComponents.length)
    throw new SalesDomainError('SALES_CONTRACT_INCOMPLETE', 'خدمت، مسافر و جزء قیمت الزامی است.');
  const serviceKeys = new Set(input.services.map(({ clientKey }) => clientKey));
  if (serviceKeys.size !== input.services.length)
    throw new SalesDomainError('SALES_SERVICE_DUPLICATE', 'کلید خدمت باید یکتا باشد.');
  for (const passenger of input.passengers) {
    if (!UUID.test(passenger.customerId) || !passenger.displayNameSnapshot.trim())
      throw new SalesDomainError('SALES_PASSENGER_INVALID', 'مشخصات مسافر کامل نیست.');
    passengerAgeCategory(passenger.birthDate, input.departureDate);
    if (!passenger.serviceClientKeys.length || passenger.serviceClientKeys.some((key) => !serviceKeys.has(key)))
      throw new SalesDomainError('SALES_PASSENGER_SERVICE_INVALID', 'تخصیص خدمت به مسافر معتبر نیست.');
  }
  const tickets = input.ticketSelections ?? [];
  for (const ticket of tickets) {
    if (!serviceKeys.has(ticket.serviceClientKey))
      throw new SalesDomainError('TICKET_NOT_AVAILABLE', 'خدمت بلیت انتخاب‌شده معتبر نیست.');
    ...
    if (arriveAt <= departAt)
      throw new SalesDomainError('TICKET_NOT_AVAILABLE', 'زمان رسیدن باید پس از حرکت باشد.');
  ...
  if (input.tripType === 'ROUND_TRIP' && (!outbound || !inbound))
    throw new SalesDomainError('RETURN_TICKET_INVALID', 'هر دو بلیت رفت و برگشت الزامی است.');
  if (
    outbound &&
    inbound &&
    (inbound.originId !== input.destinationId ||
      inbound.destinationId !== input.originId ||
      Date.parse(inbound.departureAt) < Math.max(Date.parse(outbound.arrivalAt), Date.parse(input.returnNotBefore ?? input.departureDate)))
  )
    throw new SalesDomainError('RETURN_TICKET_INVALID', 'مسیر یا زمان بلیت برگشت با قرارداد سازگار نیست.');
  if (input.hotelSelection) {
    const hotel = input.hotelSelection;
    if (!serviceKeys.has(hotel.serviceClientKey) || hotel.cityId !== input.destinationId)
      throw new SalesDomainError('SALES_HOTEL_INVALID', 'هتل باید برای شهر مقصد و خدمت انتخاب‌شده باشد.');
    if (validDate(hotel.checkOutDate, 'خروج هتل') <= validDate(hotel.checkInDate, 'ورود هتل'))
      throw new SalesDomainError('SALES_HOTEL_INVALID', 'خروج هتل باید پس از ورود باشد.');
    if (hotel.roomCount < 1 || hotel.occupancy < 1)
      throw new SalesDomainError('SALES_HOTEL_INVALID', 'تعداد اتاق و ظرفیت باید مثبت باشد.');
  }
  for (const price of input.priceComponents) {
    currency(price.currencyCode);
    if (decimalUnits(price.amount) <= 0n)
      throw new SalesDomainError('SALES_MONEY_INVALID', 'مبلغ جزء قیمت باید مثبت باشد.');
  }
...
export function validateSalesPayment(payment: SalesPaymentInput): void {
  ...
  if (decimalUnits(payment.amount) <= 0n)
    throw new SalesDomainError('SALES_MONEY_INVALID', 'مبلغ پرداخت باید مثبت باشد.');
  validDate(payment.dueAt, 'سررسید پرداخت');
  if (payment.method === 'CHECK' && !payment.check)
    throw new SalesDomainError('SALES_CHECK_INVALID', 'اطلاعات امن چک الزامی است.');
  if (payment.method !== 'CHECK' && payment.check)
    throw new SalesDomainError('SALES_CHECK_INVALID', 'اطلاعات چک فقط برای روش چک مجاز است.');
}

export function calculateSalesBalances(
  components: readonly SalesPriceComponentInput[],
  payments: readonly { amount: string; currencyCode: string; status: SalesPaymentStatus }[],
): SalesBalance[] {
  const totals = new Map<string, { total: bigint; confirmed: bigint; pending: bigint }>();
  const bucket = (code: string) => {
    const normalized = currency(code);
    const current = totals.get(normalized) ?? { total: 0n, confirmed: 0n, pending: 0n };
  ...
  for (const payment of payments) {
    ...
    else if (payment.status === 'PENDING_FINANCE_CONFIRMATION') target.pending += amount;
  }
