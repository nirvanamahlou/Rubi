...
const editor = readFileSync(
  resolve(
    process.cwd(),
    'src/modules/master-data/components/master-data-airline-baggage-editor.tsx',
  ),
  'utf8',
);
...
describe('airline baggage editing', () => {
  it('exposes adult, child and infant rules by cabin and route inside the airline form', () => {
    for (const code of ['ADT', 'CHD', 'INF'])
      expect(editor).toContain(`value: '${code}'`);
    for (const scope of ['ALL', 'DOMESTIC', 'INTERNATIONAL'])
      expect(editor).toContain(`value: '${scope}'`);
    expect(editor).toMatch(/masterDataApi\s*\.list\('cabin-classes'/);
