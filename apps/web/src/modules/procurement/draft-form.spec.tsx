...
const bootstrap: Bootstrap = {
  ...
  branches: [],
  currencies: [],
  requester: { label: 'کاربر جاری', unitId: null },
...
describe('Purchase draft accessibility and persisted input', () => {
  it('renders an incomplete draft with labeled controls, unknown estimate, and explicit unavailable documents', () => {
    ...
    expect(html).toContain('علت نامشخص بودن برآورد');
  ...
  it('retains exact decimal strings and emergency context when reopening a saved draft', () => {
    ...
    expect(html).toContain('for="line-1-acceptanceCriteria"');
  });
});
