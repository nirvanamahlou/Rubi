...
import { ticketBarcode } from '@/modules/sales/public/ticket-barcode';
  ...
  const pages = tickets
    .map((ticket) => {
      ...
      const traveler = [prefix(ticket), ticket.passengerName]
        .filter(Boolean)
        .join(' ');
      const barcode = ticketBarcode(ticket.contractNumber || '');
      const barcodeMark = barcode
        ? '<figure class="barcode"><svg viewBox="0 0 ' +
          barcode.width +
          ' 54" preserveAspectRatio="none" role="img" aria-label="Barcode ' +
          escapeHtml(barcode.value) +
          '">' +
          barcode.bars
            .map(
              (bar) =>
                ...
                '" y="0" width="' +
                bar.width +
                '" height="42" fill="currentColor"></rect>',
            )
            .join('') +
          '</svg><figcaption>' +
          escapeHtml(barcode.value) +
      ...
      return (
        ...
        (ticket.transferDirections.length
          ? '<div class="transfer">TRANSFER INCLUDED: ' +
            ...
            '</div>'
          : '') +
        '<div class="warning"><strong>PRESENCE 03:00 BEFORE FLIGHT TIME AT THE AIRPORT IS MANDATORY</strong><strong lang="fa" dir="rtl">حضور در فرودگاه ۳ ساعت قبل از پرواز الزامی است.</strong></div>' +
        barcodeMark +
        '</div><footer><span>FLY FURTHER TOGETHER</span><span>' +
        escapeHtml(branding.name) +
        '</span></footer></article>'
      );
    })
