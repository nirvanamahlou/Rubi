...
تب و دکمهٔ تکراری «تنظیمات بخش‌ها» از مدیریت سیستم حذف شدند. همهٔ کارت‌های تنظیمات و عملیات ویرایش نسخه‌دار حفظ شده‌اند و هنگام بازبودن یک کارت، تب «نمای کلی» فعال می‌ماند. تست اختصاصی، ESLint، typecheck و build تولیدی Web موفق‌اند. یک تست قراردادی قدیمیِ مستقل برای لینک‌های IAM/Legal Entity پیش از این تغییر با source حاضر ناسازگار بود؛ خارج از محدوده باقی ماند. Migration، API، داده، مجوز و وابستگی‌ها تغییری نکردند.
