...
بازطراحی حرکتی صفحهٔ ورود آمادهٔ بررسی است: همان تصویر فعلی هواپیمای پس‌زمینه از راست وارد می‌شود، با توقف نرم در جای اصلی می‌ایستد و پس از آن نشان ابری «NOORA» روی پس‌زمینه نمایان و ماندگار می‌شود. هواپیمای SVG و کارت جداگانه حذف شده‌اند؛ حالت کاهش حرکت پیاده شده و منطق احراز هویت دست‌نخورده است. ۲ تست هدفمند، lint، typecheck و build تولیدی Web موفق‌اند و پیش‌نمایش پورت مستقل ۳۲۰۱ پاسخ HTTP 200 دارد.

## 2026-09-17 — SALES-CONTRACTS-REDESIGN-0917 — PC-A — READY_FOR_REVIEW

بازطراحی ظاهری فهرست قراردادهای فروش آمادهٔ بررسی است: کارت جست‌وجوی فشرده، جدول با شناسه و وضعیت‌های خواناتر و عملیات مرتب هر قرارداد. منطق قرارداد، پرداخت، خروجی، داده و API تغییر نکرده‌اند. تست هدفمند ۸ مورد، lint و typecheck Web موفق‌اند.
...
## 2026-09-17 — پوشش دادهٔ دمو برای چهار نمودار Dashboard

دادهٔ نمونهٔ محلی Dashboard/Reporting با همان پیشوند قابل‌پاک‌سازی
`LOCAL_DEMO_DASHBOARD_REPORTING_` دوباره‌سازی شد تا چهار خروجی در Dashboard قابل مشاهده
باشند: روند فروش تقویمی، صف اقدامِ رزرو/پرداخت/صدور، قیف تصمیمِ رزرو تا صدور، و مقایسهٔ
عملکرد کارشناسان. این خروجی‌ها از Projection تأییدشدهٔ fact سفر تغذیه می‌شوند و عدد داخل
Frontend جعل نمی‌شود. برای جلوگیری از ورود دادهٔ غیرواقعی به نسخهٔ نهایی، دستور
`pnpm reporting:demo:clear` فقط رکوردهای پیشونددار را حذف می‌کند و
`pnpm reporting:demo:verify-clean` در صورت باقی‌ماندن حتی یک رکورد دمو با خطا متوقف
می‌شود. در حال حاضر ۱۸۰ رکورد نمونه عمداً برای مشاهده روی localhost نگه داشته شده‌اند.

API محلی روی ۴۰۰۰ و Web روی ۳۰۰۰ بازسازی و راه‌اندازی شده‌اند؛ تست هدفمند Reporting
(۶ مورد)، TypeScript و lint API موفق‌اند. Migration، Schema، Permission، دادهٔ عملیاتی،
Dependency و Lockfile تغییر نکرده‌اند.

## 2026-09-17 — ناوبری Dashboard با زیرصفحه‌های بسته در ورود

در ورود اولیه به Dashboard، همهٔ گروه‌های زیرصفحه در Sidebar بسته‌اند و تنها پنج صفحهٔ اصلی قابل مشاهده‌اند. هر گروه فقط با انتخاب کنترل بازکردن خودش گسترش می‌یابد؛ وضعیت صفحهٔ فعال نیز به‌تنهایی هیچ زیرصفحه‌ای را باز نمی‌کند.

## 2026-09-17 — حفظ پنل KPI هنگام پیکربندی گزارش مرتبط

باز شدن فرم پیکربندی گزارش مرتبط دیگر پنل توضیح KPI را نمی‌بندد. فرم گزارش در Dialog روی Dashboard باز می‌شود و Drawer تعریف KPI زیر آن فعال باقی می‌ماند؛ پس از بستن فرم، کاربر در همان KPI و همان وضعیت فیلتر باقی می‌ماند.

## 2026-09-17 — پیکربندی گزارش در همان صفحهٔ Dashboard

دکمهٔ گزارش مرتبط در پنل KPI و نمودارهای Dashboard دیگر کاربر را به `/reports` نمی‌برد. همان فرم پیکربندی استاندارد گزارش، با فیلترها، اجرای نتیجه، ذخیره و خروجی Excel، در دیالوگ روی Dashboard باز می‌شود. آدرس و فیلترهای Dashboard دست‌نخورده باقی می‌مانند؛ نموداری که گزارش مشخصی در کاتالوگ ندارد، به‌جای ناوبری مبهم غیرفعال است.

## 2026-09-17 — کنتراست CTA گزارش مرتبط در پنل KPI

متن و آیکون دکمهٔ «رفتن به فرم پیکربندی گزارش مرتبط» اکنون در حالت عادی، hover و focus-visible به‌صورت قطعی سفید هستند. اولویت رنگ روی خود لینک و تمام محتوای داخلی اعمال شده تا variant پیش‌فرض Button باعث بازگشت رنگ تیره نشود.

## 2026-09-17 — ادغام Backend مدیریت سیستم

Backend مدیریت سیستم PC-B ادغام شد: قراردادهای نسخه‌دار، APIهای محافظت‌شدهٔ تنظیمات و شماره‌گذاری، مدیریت نشست و اعلان، قالب‌ها، feature flagها، سلامت و درخواست پشتیبان‌گیری به‌همراه Migration و seed مجوزها اکنون در شاخهٔ نهایی حاضرند. Dashboard، Reporting و بهبودهای رزرو بدون بازنویسی حفظ شده‌اند. Migration صرفاً در مخزن ثبت شده و روی دادهٔ محلی اعمال نشده است.

## 2026-09-17 — ادغام پنل فشردهٔ عملیات رزرو

بهبود PC-A برای پنل عملیات قراردادهای رزرو ادغام شد: پنل در دسکتاپ جمع‌وجورتر است، گروه‌های عملیات چیدمان شبکه‌ای واکنش‌گرا دارند و نخستین قراردادِ فهرست فیلترشده به‌طور پیش‌فرض انتخاب می‌شود. قابلیت‌های مدیریت سیستم PC-B نیز از `develop` دریافت شد. تغییرهای Dashboard و Reporting این worktree حفظ شده‌اند.

## 2026-09-17 — روند تقویمی KPI Cardهای Dashboard

روند KPI Cardها دیگر به ۸ بخش ثابت تقسیم نمی‌شود. API در منطقهٔ زمانی `Asia/Tehran` برای امروز bucket ساعتی، برای هفته و ماه bucket روزانه، برای فصل bucket هفتگی و برای سال مرز ماه‌های فارسی را برمی‌گرداند. بازهٔ سفارشی نیز متناسب با طول بازه دانه‌بندی می‌شود. هیچ bucket آینده‌ای تولید نمی‌شود؛ بنابراین صفر ساختگی به‌عنوان دادهٔ مشاهده‌شده در روند نمایش نمی‌گیرد. ۸ تست Reporting، TypeScript و lint API موفق‌اند؛ API Contract، Migration، Schema، دادهٔ عملیاتی، وابستگی و Lockfile تغییر نکرده‌اند.

## 2026-09-17 — کنتراست گزینه‌های فیلتر Dashboard

متن گزینه‌های فیلتر Dashboard در hover و focus-visible روی پس‌زمینهٔ آبی، سفید نمایش داده می‌شود. این رفتار برای فیلترهای جست‌وجوپذیر و انتخاب بازهٔ زمانی یکسان است و تیک گزینهٔ انتخاب‌شده نیز در hover سفید می‌شود.

## 2026-09-17 — فیچرهای استفاده‌شده در فرمول KPI

میان «فرمول و قاعده محاسبه» و «حذف‌ها و محدودیت‌های محاسبه» در پنل تعریف KPI، فهرست بولت‌دارِ فیچرهای واقعیِ به‌کاررفته در فرمول افزوده شد. هر مورد نام فنی و توضیح کوتاهِ داده‌ای دارد؛ از جمله `recognized sale revenue` برای اقلام خدمتِ قراردادهای نهایی و `matched purchase cost` برای هزینه‌های خرید تأییدشده و تطبیق‌خورده. این فهرست مستقیماً از `definition.source` ساخته می‌شود و برای منبع تازه، بدون جعل مقدار یا داده، توضیح fallback دارد. API، Reports، دیتابیس، Migration و دادهٔ عملیاتی تغییر نکرده‌اند. ۱۶ تست Dashboard، TypeScript و lint Web موفق‌اند.

## 2026-09-17 — ساده‌سازی پنل تعریف قابل ممیزی KPI

پنل KPI اکنون عنوان بزرگ و مرکزچین شاخص، نام فنی، تعریف و هدف، تصمیم، مقدار دقیق، فرمول و محدودیت‌ها را نشان می‌دهد. تگ‌های شناسه و گزارش، منابع داده و حاکمیت از UI حذف شدند. دکمهٔ Close به ضربدر دسترس‌پذیر تغییر کرد و CTA فرم گزارش با متن و آیکون سفید نمایش می‌یابد. تست Dashboard، TypeScript و lint Web موفق‌اند.

## 2026-09-17 — اصلاح هندسهٔ Donut

ریشهٔ خطوط جدا از حلقه، تفاوت مختصات SVG با حلقهٔ CSS بود. حلقه و خطوط اکنون با SVG واحد رسم می‌شوند؛ ضخامت حلقه متناسب با نمونه کاهش یافته و برچسب‌ها بیرونی با خط خاکستری کوتاه هستند. مقدار مرکز از دادهٔ واقعی حفظ می‌شود. همچنین مسیر تصویرهای header از Proxy ورود مستثنا شد؛ پیش‌تر بهینه‌ساز تصویر به صفحهٔ ورود پاسخ می‌گرفت و تصویر در سربرگ دیده نمی‌شد. تست Dashboard/Proxy، TypeScript و lint موفق‌اند.

## DASHBOARD-DONUT-LEADER-LINES-AND-HEADER-ART-0916 — تکمیل محلی

- برچسب‌های Donut از خود حلقه جدا شدند: خط راهنما از لبهٔ بخش آغاز می‌شود، مسیر شعاعی و اتصال تا ستون برچسب دارد و عنوان و مقدار/درصد واقعی در دو سطر با فاصلهٔ امن از نمودار دیده می‌شوند. داده، درصد، مجموع و قاعدهٔ تبدیل دسته‌های بیش از شش مورد به Bar تغییری نکرده‌اند.
- برای پنج خانوادهٔ اصلی Dashboard تصویر دکوراتیو مینیمال و بدون متن تولید شد: نمای مدیریتی، عملکرد تجاری، مالی و خزانه‌داری، مشتری و رشد، و سرمایه انسانی. هر زیرصفحه تصویر خانوادهٔ خود را می‌گیرد. تصاویر local هستند و `next/image` آن‌ها را فقط برای صفحهٔ فعال، با WebP و کیفیت 45 بهینه می‌کند؛ درخواست خارجی، dependency، API، Projection، Reports، Migration و دادهٔ عملیاتی تغییر نکرده‌اند.

## DASHBOARD-CALENDAR-MONEY-FILTER-PANELS-0916 — تکمیل محلی

- مبلغ هر ارز در KPI Card اکنون یک واحد غیرقابل‌شکستن است: نماد با فاصلهٔ کوتاه در چپ مقدار می‌آید و روی کارت به شکل فشرده (برای ریال میلیارد/میلیون/هزار و برای ارزهای دیگر B/M/K) نمایش داده می‌شود. مقدار دقیق بدون فشرده‌سازی در Tooltip و پنل تعریف قابل ممیزی KPI باقی است؛ FX یا جمع میان‌ارزی ساخته نشده است.
- بازه‌های امروز، هفته، ماه، فصل و سال از مرزهای تقویم فارسی در `Asia/Tehran` ساخته می‌شوند؛ هفته از شنبه شروع می‌شود. مقایسه با بازهٔ قبل هم‌طول و برای هر ارز مستقل باقی است.
- Sidebar غیرچسبان Dashboard اکنون در بالا دو Tab «فضای کار داشبوردها» و «فیلترهای داشبورد» دارد و تنها محتوای Tab منتخب را نمایش می‌دهد؛ بدین‌ترتیب پیمایش صفحات و کنترل‌های فیلتر در یک ستون بلند تکراری قرار نمی‌گیرند. typecheck و lint API/Web، build API و بررسی مستقیم مرزهای تقویمی موفق‌اند. اجرای Vitest هدفمند در runner محلی طی زمان مجاز کامل نشد. Migration، Schema، دادهٔ عملیاتی، Reports، وابستگی و Lockfile تغییر نکردند.

## DASHBOARD-KPI-COMPARISON-VISIBILITY-0916 — تکمیل محلی

- ریشهٔ حذف تگ رشد این بود که Frontend ساخت تگ را به وجود `comparison` یا `trend` در پاسخ Projection شرطی کرده بود؛ در پاسخِ فاقد یکی از این فیلدها، حتی برای بازه‌های هفته/ماه، هیچ وضعیت قابل مشاهده‌ای رندر نمی‌شد.
- کارت اکنون برای هر KPI دارای خروجی همواره تگ وضعیت دورهٔ قبل هم‌طول دارد: مقدار واقعی API با فلش/درصد نمایش می‌یابد و در نبود comparison یا صفر بودن مبنای قبل، تگ خنثی «—» همراه Tooltip قابل ممیزی باقی می‌ماند. مقدار یا درصد ساختگی تولید نمی‌شود. typecheck، lint و build تولیدی Web موفق‌اند؛ Vitest هدفمند به علت نبود باینری محلی آن اجرا نشد. API، قرارداد، دیتابیس، Migration، Reports و وابستگی تغییری نکردند.

## DASHBOARD-VISUAL-READABILITY-CLEANUP-0916 — تکمیل محلی

- کم‌رنگ‌ترین سطح طیف رتبه‌ای از `#dbeafe` به `#93c5fd` تغییر کرد تا خوانایی Bar و Donut حفظ شود. دو پنل «قرارداد state و دسترسی» و «تصمیم‌های باز و metadata» از Frontend Dashboard حذف شدند.
- تگ رشد KPI در سطر خروجی، ابتدا comparison مستقل همان ارز و سپس comparison تک‌ارزی سازگار را استفاده می‌کند. اگر دادهٔ دورهٔ قبل برای درصد وجود نداشته باشد، تگ با خط تیره و Tooltip صریح باقی می‌ماند؛ درصد یا مبلغ ساختگی تولید نمی‌شود. تست Dashboard (۱۶)، lint و TypeScript Web موفق‌اند.

## DASHBOARD-KPI-VALUE-ROW-ALIGNMENT-0916 — تکمیل محلی

- سطر خروجی KPI اکنون در RTL سه ستون دارد: تگ رشد/کاهش/ثبات در سمت راست، مقدار عددی در میانه و نماد ارز در سمت چپ. نماد و مبلغ با `bdi` جدا نگهداری می‌شوند تا ترتیب اعداد و نشانه‌ها پایدار بماند.
- API، قرارداد، دیتابیس، Migration و Reports تغییری نکردند. تست Dashboard (۱۶)، lint و TypeScript Web موفق‌اند.

## DASHBOARD-RANKED-CHARTS-COMPARISON-BADGES-0916 — تکمیل محلی

- Barهای مقایسه‌ای و بخش‌های Donut به‌جای رنگ‌های نامرتبط، طیف آبی تیره‌به‌روشن بر پایهٔ مقدار دارند؛ بزرگ‌ترین مقدار تیره‌ترین رنگ را می‌گیرد و همان رنگ در legend تکرار می‌شود.
- علت نبود تگ KPIهای پولی چندارزی رفع شد: Projection اکنون درصد دورهٔ قبل هم‌طول را جداگانه برای هر `currencyCode` بازمی‌گرداند و کارت تگ فلش/خط ثبات و درصد هر ارز را روبه‌روی عنوان نمایش می‌دهد. FX یا جمع میان‌ارزی انجام نمی‌شود. تست API (۵)، Web (۱۶)، lint و TypeScript هر دو برنامه موفق‌اند.

## DASHBOARD-KPI-TREND-TREATMENT-0916 — تکمیل محلی

- نشان رشد/کاهش/ثبات KPI به روبه‌روی عنوان در سربرگ کارت منتقل شد و با فلش یا خط تیره و درصد نمایش داده می‌شود. Sparkline پایین کارت ۵۶px ارتفاع دارد و برای هر ارز، سطح گرادیانی همرنگ خط و legend قابل‌دسترسی نشان می‌دهد.
- متن «هر ارز مستقل و بدون تبدیل نمایش داده می‌شود» از ظاهر کارت حذف شد؛ شرح مقیاس مستقل فقط برای دسترس‌پذیری در `aria-label` روند مانده است. API، داده، قرارداد، Migration و Reports تغییری نکردند. تست Dashboard (۱۶)، lint و TypeScript Web موفق‌اند.

## DASHBOARD-VISUAL-CHROME-CLEANUP-0916 — حذف جزئیات تکراری نمودار

- مقایسهٔ «نسبت به دوره قبل»، بلوک «روند بازهٔ انتخاب‌شده» و footer «منبع» از همهٔ کارت‌های نمودار Dashboard حذف شدند. روند و مقایسه فقط در KPI Cardها نمایش دارند.
- نمودارهای اصلی، legendهای تحلیلی، تصمیم باز و دکمهٔ گزارش مرتبط حفظ شدند. تست Dashboard Web (۱۶)، lint و TypeScript موفق‌اند؛ API یا داده تغییری نکرده است.

## DASHBOARD-KPI-MULTI-CURRENCY-TRENDS-0916 — نمایش مبلغ و روند چندارزی

- کارت‌های KPI پولی مبلغ‌ها را مرکزچین و با نماد ارز نمایش می‌دهند: `$` برای USD و `﷼` برای IRR/IRI. تبدیل یا جمع چند ارز انجام نمی‌شود.
- Projection Dashboard برای هر ارز موجود سری trend جداگانه برمی‌گرداند؛ کارت خطوط رنگی و legend نماد ارز را بدون نیاز به انتخاب فیلتر ارز نشان می‌دهد. برای پرهیز از مقایسهٔ نادرست ارزش دلار و ریال، هر خط در مقیاس نسبی همان ارز رسم می‌شود.
- تست Reporting API (۵)، تست Dashboard Web (۱۶)، lint و TypeScript API/Web موفق‌اند. تغییر Migration یا دادهٔ محلی ندارد.

## DASHBOARD-BI-VISUAL-SYSTEM-0916 — بازطراحی خروجی‌های تحلیلی

- سیستم بصری Dashboard بدون تغییر در Projection یا داده بازطراحی شد: Barهای رتبه‌بندی افقی، Combo واقعی ستون/خط میانگین، Donut با مجموع مرکزی و Legend عددی، و جدول متراکم با سهم مقایسه‌ای.
- چیدمان کارت‌های نمودار در هر ردیف هم‌ارتفاع است و KPIها روند و تغییر دوره‌ای موجود را حفظ می‌کنند. دسترس‌پذیری با خلاصهٔ متنی/جدولی و عنوان‌های قابل خواندن حفظ شد.
- lint، TypeScript، ۱۴۶۷ تست Web و build تولیدی ۴۶ مسیر موفق‌اند. Runtime یکپارچه از Worktree مرجع روی پورت ۳۰۰۰ و API روی ۴۰۰۰ فعال است؛ مرورگر فعلی پیش از ورود قرار دارد، بنابراین پذیرش تصویری Dashboard احرازشده به نشست کاربر وابسته است.

## REPORTING-OPERATIONS-RECENT-30-0916 — سقف تاریخچه اجرا و خروجی

- سکشن‌های «اجراها» و «خروجی‌ها» فقط ۳۰ رکورد اخیر را بر اساس `createdAt DESC` دریافت و نمایش می‌دهند؛ رکوردهای قدیمی در دیتابیس برای ممیزی باقی می‌مانند و حذف فیزیکی نشده‌اند.
- API و Frontend هر دو این سقف را enforce می‌کنند. تست Reports Web (۲ مورد)، lint و TypeScript موفق‌اند.

## REPORTING-PIE-CENTER-0916 — مرکزچینی نمودار دایره‌ای

- نمودار دایره‌ای و کد ارز اکنون در بالای هر کارت و در مرکز افقی آن قرار دارند؛ Legend زیر نمودار و جدا از آن باقی می‌ماند.
- تست Reports Web (۱۶ مورد)، lint و TypeScript موفق‌اند؛ Web روی پورت ۳۰۰۰ فعال است.

## REPORTING-PIE-LEGEND-LAYOUT-0916 — نمودار دایره‌ای

- در هر کارت نمودار دایره‌ای، خود دایره و کد ارز در بالای کارت قرار می‌گیرند و راهنمای رنگ‌ها با یک جداکنندهٔ مشخص زیر نمودار نمایش داده می‌شود؛ Legend دیگر در کنار یا روی نمودار نیست.
- اسکرول داخلی نمودار برای داده‌های زیاد حفظ شده است. ۱۶ تست هدفمند Reports Web، lint و TypeScript موفق‌اند.

## REPORTING-RESULT-SCROLL-PAGING-0916 — نمایش نتیجه گزارش

- جدول نمای نتیجه فقط ۵ رکورد در هر صفحه دریافت و نمایش می‌دهد و صفحه‌بندی قبلی/بعدی صرفاً در حالت جدول دیده می‌شود.
- نمودارهای میله‌ای افقی، ستونی و دایره‌ای تا سقف ۱۰۰ رکورد پیش‌نمایش را دریافت می‌کنند و با اسکرول داخلی افقی/عمودی قابل مشاهده‌اند؛ تغییر بین جدول و نمودار اندازه درخواست را متناسب با همان نما بازتنظیم می‌کند.
- ۱۶ تست هدفمند Reports Web، lint و TypeScript موفق‌اند؛ Web و API روی پورت‌های ۳۰۰۰ و ۴۰۰۰ پاسخ HTTP 200 دارند.

## REPORTING-XLSX-SECTION-SPACING-0916 — فاصله‌بندی خروجی Excel

- در XLSX گزارش‌ها سطر ۱ برای نام گزارش و زمان، سطر ۲ خالی، سطرهای ۳ و ۴ برای عنوان/مقدار فیلترها، سطر ۵ خالی و جدول داده از سطر ۶ است.
- Freeze Pane و محدودهٔ Table/AutoFilter به سطر ۶ منتقل شدند تا فاصله‌ها خارج از جدول بمانند. CSV و PDF تغییر نکردند؛ ۲ تست هدفمند، lint و TypeScript API موفق‌اند.

## REPORTING-XLSX-LAYOUT-0915 — خروجی Excel گزارش‌ها

- فرم پیکربندی Reports فقط XLSX می‌فرستد. workbook در سطر اول نام گزارش و زمان تولید UTC، پس از یک سطر خالی در سطرهای ۳ و ۴ عنوان/مقدار فیلترهای snapshot و پس از سطر خالی دوم از سطر ۶ جدول داده را نشان می‌دهد؛ کد گزارش و Projection از محتوای workbook حذف شدند.
- Excel همهٔ ردیف‌های مجاز گزارش را بدون برش Preview صادر می‌کند. اعداد با فرمت هزارگان و شناسه‌ها به‌صورت متن‌اند؛ مبالغ بیش از دقت ۱۵رقمی Excel با مقدار دقیق و جداکننده هزارگان به‌صورت متن حفظ می‌شوند. اعتبارسنجی بازشدن workbook با داده و بدون داده انجام شد.

## REPORTING-CASH-POSITION-0915 — امکان‌سنجی موقعیت نقدینگی

- درخواست مدیرعامل برای وجه موجود امروز، تعهدات پرداخت هفت روز آینده و نیاز تزریق/برداشت بررسی شد. کارت `RPT-037` با عنوان درخواستی در کاتالوگ API و Web فقط به‌صورت `PENDING_CONNECTION` ثبت شده است؛ producer `reporting_cash_position_facts_v1` هنوز منتشر نشده و هیچ عدد نمونه‌ای به‌عنوان وجه قابل برداشت ارائه نمی‌شود.
- در PostgreSQL محلی قرارداد/دریافت مشتری صفر و جداول حساب تسویه/خرید/پرداخت تأمین‌کننده هنوز نصب نشده‌اند؛ مانده تطبیق‌یافته بانک، وضعیت وصول چک و سررسید تعهد مصوب در منابع فعلی موجود نیست. Schema/Migration/Finance و runtime مشترک تغییر نکردند. تعریف، فهرست فیچرهای لازم، قرارداد grain/ارز و احتیاط مالی در [REPORTING-CASH-POSITION-0915](tasks/REPORTING-CASH-POSITION-0915.md) آمده است.
- تست‌های کاتالوگ API و Web (۱۲ مورد)، lint محدوده، TypeScript و build API/Web موفق‌اند. انتشار عددی منوط به تصمیم/کار مستقل مالک Finance و Reservations درباره مدل مانده/سررسید/Projection مصوب است.
- در درخواست بعدی مالک، commit گزارش نقدینگی بدون بازگرداندن ماژول‌های دیگر روی شاخهٔ اجرایی `codex/pc-c-dashboard-reporting-latest@ba7eddc8` فعال شد. Web از همین worktree روی ۳۰۰۰ و API بازساخته‌شده روی ۴۰۰۰ اجرا می‌شوند؛ Login و Health هر دو HTTP 200، تست کاتالوگ ۱۲/۱۲ و Web TypeScript موفق‌اند. نمایش صفحهٔ محافظت‌شده به نشست معتبر نیاز دارد.
...
## 2026-09-16 — اتصال‌های عملیاتی غیررزرواسیونی خرید — PC-B

اتصال‌های باقی‌ماندهٔ Procurement بدون افزودن سکشن یا UI تازه تکمیل شدند. Settings
سیاست تأیید نسخه‌دار شعبه/واحد/دسته/ارز را نگه می‌دارد و نبود سیاست همچنان fail-closed
است. رخدادهای workflow در همان تراکنش به وظیفهٔ مسئول یا تأییدکننده تبدیل می‌شوند.
فاکتور تطبیق‌شده در کارتابل Finance قابل تأیید، برگشت برای اصلاح و پرداخت مرحله‌ای با
Decimal، حساب تسویه و روش پرداخت است و نتیجه به مالک Procurement بازمی‌گردد. مرجوعی
کالای پذیرفته‌شده نیز یک اصلاح مالی نسخه‌دار می‌سازد. سفارش صادرشده در outbox امن
تأمین‌کننده با HMAC، idempotency، replay guard و retry قرار می‌گیرد؛ تا زمان ارائه URL
و Secret بیرونی، ارسال واقعی انجام نمی‌شود. گزارش خرید وضعیت مالی را از همین رکوردهای
پایدار محاسبه می‌کند. اتصال‌های Reservations بنا به درخواست مالک خارج از این تغییرند.

Migration افزایشی چهار جدول سیاست، وظیفه، revision مالی و inbox تأمین‌کننده را ایجاد
می‌کند و نبود migration قدیمی حساب تسویه مالی را به‌صورت idempotent جبران می‌کند.
تمرین migration روی پایگاه اختصاصی PostgreSQL 18 در تراکنش موفق بود. ۳۰ تست عملیاتی
Procurement و ۲۱ تست هدفمند قرارداد/Finance/Settings/Tasks/Integration، به‌همراه Prisma validate/generate،
lint و typecheck و build تولیدی Contracts/Database/API پاس شده‌اند.

## 2026-09-16 — اتصال واحد سازمانی فرم خرید به منابع انسانی — PC-B

فرم درخواست خرید، درخواست‌کننده را از کارکنان فعال HR و واحد سازمانی را از واحدهای فعال ساختار سازمانی و پرونده‌های کارکنان همان شعبه می‌خواند. انتخاب واحد فهرست کارکنان را محدود می‌کند و اعتبارسنجی سرور اجازه ثبت واحد ناسازگار با کارمند را نمی‌دهد. فیلد نمایشی اولویت حذف شد و مقدار پیش‌فرض داخلی برای سازگاری قرارداد v1 باقی ماند. عنوان بخش «منشأ درخواست و پیوست‌ها» و نام فیلدهای ارجاع رزرواسیون نیز روشن‌تر شد. این تغییر Schema، Migration، Dependency یا Grant ندارد.

## 2026-09-16 — تکمیل داده‌های نمایشی همهٔ سکشن‌های خرید — PC-B

در پایگاه محلی، ۳۱ نام تأمین‌کننده از عبارت «آزمایشی» پاک و عنوان درخواست قدیمی E2E به عنوان کسب‌وکاری تبدیل شد. ده پیش‌نویس خرید واقعی‌نما نیز از مسیر رسمی فرم و API با شماره‌های خودکار ثبت شدند. fixture خواندنی سکشن‌های تأیید، پیشنهاد، سفارش، دریافت و فاکتور با چهار پروندهٔ تازه برای هر سکشن تکمیل شد؛ این رکوردها فقط هنگام خالی بودن صف نمایش داده می‌شوند و فرمان خرید، سفارش خارجی یا پرداخت ایجاد نمی‌کنند. Schema، Migration، Dependency، Permission و فایل‌های گزارش untracked تغییری نکردند.

## 2026-09-15 — زیرساخت اتصال‌های Backend خرید — PC-B

در Procurement، خواندن سیاست تأیید نسخه‌دار از artifact مصوب Settings با تطبیق
شعبه/واحد/دسته/ارز و رد امن فایل نامعتبر یا سیاست مبهم اضافه شد؛ چون حساب‌ها و
سقف‌های تأیید هنوز تعیین نشده‌اند، runtime فعلی سیاست ندارد و ارسال مسدود می‌ماند.
منشأ فاکتور عمومی تطبیق‌شده از `ProcurementPublicService` با scope شعبه برای
Finance قابل خواندن است؛ consumer/ack و پرداخت Finance هنوز نیازمند قرارداد PC-A
است. outbox intent سفارش تأمین‌کننده فقط پس از تأیید و صدور داخلی ساخته می‌شود
و تا معرفی API بیرونی `BLOCKED` است. projection امن رخدادهای workflow و intent
برای consumerهای آینده Tasks/Integrations از همان public boundary قابل خواندن‌اند؛
تحویل واقعی رخداد به Tasks و adapter رزرواسیون هنوز به قرارداد مالکشان وابسته است.
هیچ سکشن Web، سفارش خارجی، پرداخت، مجوز خودکار، Migration یا Dependency اضافه نشد.
دو تست سیاست و هر ۳۰ تست سرویس روی پایگاه اختصاصی PostgreSQL، typecheck، lint و
build API موفق‌اند. اجرای نخست کل تست‌ها زیر بار build هم‌زمان به timeout خورد؛
اجرای نهایی بدون بار موازی و با مهلت مناسب، همهٔ موارد را گذراند.

## 2026-09-15 — انتخاب‌گرهای داده‌محور خرید — PC-B

منوهای انتخاب درخواست، مسئول، عملیات پرونده، مرجع‌های سفارش/فاکتور، فیلترها و خروجی خرید از Select مشترک روبی استفاده می‌کنند؛ گزینه‌های مرجع از APIهای HR، اطلاعات پایه و خرید می‌آیند. نوع و دسته خرید، واحد سنجش و دورهٔ خدمت از مقادیر درخواست‌های ذخیره‌شدهٔ خود کاربر در همان شعبه پیشنهاد می‌شوند و ورود مقدار تازه همچنان ممکن است، چون کاتالوگ عمومی مستقلی برای آن‌ها وجود ندارد. محل تحویل/دریافت عملیات نیز شعبه‌های موجود را پیشنهاد می‌دهد و محل دیگر قابل ثبت است. typecheck، lint، شش تست هدفمند و build تولیدی Web با ۴۶ مسیر موفق‌اند. بدون Schema، Migration، API، Dependency یا Seed؛ فقط frontend و تست خرید تغییر کرده‌اند.

## 2026-09-15 — کوتاه‌کردن مسیر بالای خرید — PC-B

breadcrumb صفحهٔ خرید اکنون فقط «روبی / خرید و تأمین» را نشان می‌دهد؛ عنوان «میزکار خرید» یا عنوان بخش جاری در مسیر تکرار نمی‌شود. در بخش‌های فرعی، پیوند «خرید و تأمین» همچنان به میزکار برمی‌گردد و URL بخش/دکمهٔ Back حفظ شده است. typecheck، lint، دو تست هدفمند و build تولیدی Web با ۴۶ مسیر موفق‌اند. این تغییر فقط در frontend خرید است؛ قرارداد، داده، API و Schema تغییر نکردند.

## 2026-09-15 — حذف هشدار و کنترل‌های صف از صفحهٔ خرید — PC-B

هشدار نمایشی سیاست تأیید، متن توضیحی دادهٔ آزمایشی و نوار نه صف کاری از workspace خرید حذف شدند. میزکار همچنان فهرست «درخواست‌های من» را با `queue=own` و بخش تأیید فهرست `queue=approvals` را از API می‌خواند؛ دسترسی به بخش‌ها از کارت‌های میزکار و پیوندهای مرتبط برقرار است. رکورد نمونه فقط در فهرست خالی ظاهر می‌شود و در هر ردیف برچسب نمونه/فقط نمایش دارد. گیت سیاست تأیید در backend و غیرفعال بودن ارسال در فرم تغییری نکردند. typecheck، lint، چهار تست هدفمند و build تولیدی Web با ۴۶ مسیر موفق‌اند. بدون API، Schema، Migration، Dependency یا تغییر داده.

## 2026-09-15 — ساده‌سازی صفحهٔ خرید و دادهٔ نمونهٔ آزمایشی — PC-B

نوار هشت‌گزینه‌ای بالای workspace خرید و بنر «خرید را از یک مسیر دنبال کنید» حذف شدند. ورود به بخش‌ها از کارت‌های میزکار، پیوند بازگشت در breadcrumb و پیوندهای دو بخش مرتبط باقی است. در صف خالی شخصی و فهرست درخواست‌ها، پنج پروندهٔ آزمایشی واقع‌نما فقط برای نمایش ظاهر نشان داده می‌شوند؛ نمونه‌ها برچسب دارند، به API یا پایگاه داده ارسال نمی‌شوند، عملیات ندارند و به محض وجود پروندهٔ واقعی پنهان می‌شوند. typecheck، lint، چهار تست هدفمند و build تولیدی Web با ۴۶ مسیر موفق‌اند. کد فقط در frontend خرید تغییر کرد؛ بدون Schema، Migration، API، دادهٔ واقعی یا Dependency.

## 2026-09-15 — رفع خطای مجوز خرید در حساب محلی — PC-B

خطای ۴۰۳ bootstrap خرید از نبود مجوز Procurement در نقش‌های نشست فعلی `nirvana` بود؛ نقش ثبت درخواست `staff` در پایگاه عملیاتی صفر عضو داشت. همان حساب فعال و شعبه‌دار به‌صورت افزایشی فقط به `staff` اضافه شد و Audit محلی ثبت شد. پنج مجوز این نقش شامل مشاهده/ثبت/ویرایش/ارسال/لغو درخواست خود است؛ نقش‌های قبلی و شعب حفظ شدند و هیچ اختیار تأیید یا سفارش داده نشد. اکنون کاتالوگ محلی یک عضو `staff` و صفر عضو برای هر یک از `procurement_approver` و `procurement_buyer` دارد. IAM در هر درخواست نقش‌های جاری را دوباره می‌خواند؛ بررسی داخل مرورگر پس از بازخوانی صفحه و داشتن نشست معتبر لازم است. کد، Migration و build تغییری نکردند.
...
زیر سرمایه انسانی است؛ فرم درخواست، فهرست‌ها، ناوبری و URL بخش‌ها اصلاح شده‌اند و
گزارش‌های نمایشی خرید حذف شده‌اند. درخواست‌کننده از دادهٔ ذخیره‌شدهٔ HR و محل
تحویل از شعبه‌های عمومی انتخاب می‌شود. پنج migration خرید و migration افزایشی
منتشرشدهٔ Finance ابتدا روی کپی پایگاه و سپس پایگاه محلی اعمال شدند؛ backup پیش
...
پیگیری فرم تأمین‌کننده در 2026-09-16 نام فارسی/انگلیسی را در یک کنترل «نام تأمین‌کننده» ادغام و کشور/شهر را از فرم ایجاد حذف کرد. سازمان موجود ابتدا جست‌وجو می‌شود و ثبت سازمان تازه فقط در نبود نتیجه در دسترس است؛ پس از ثبت سازمان تازه نیز فرم مخاطب اصلی با کانال تلفن باز می‌شود تا شماره تماس به همان سازمان و سپس Supplier متصل شود. داده‌های قدیمی و قرارداد API سازگار باقی می‌مانند و Schema/Migration/Dependency تغییری ندارد.

فرم سفارش سکشن پنجم برای داده‌های نمایشی خالی دیگر انتخاب‌گر بدون گزینه ندارد: همان پرونده‌های روی صفحه از ردیف یا انتخاب‌گر باز می‌شوند و فرم مخصوص پیشنهاد منتخب، موعد تحویل، شعبه و شرایط پرداخت را نشان می‌دهد. ذخیره این پرونده‌ها فقط در نشست مرورگر است؛ سفارش واقعی، تأیید و صف بیرونی همچنان از قرارداد و مجوز Backend عبور می‌کنند.
...
در 2026-09-15 یک اجرای مستقیم Next از checkout اصلی `C:/Users/admin/Rubi` روی ۳۱۰۰
Gateway را جایگزین کرده بود و نسخهٔ همه‌بخش‌ها دیده نمی‌شد. پس از بررسی مالکیت پورت و
بازگرداندن Gateway، دو تغییر تازهٔ `develop` در رزرواسیون و پروفایل نیز به شاخهٔ
تجمیعی وارد شدند. build قراردادها، API و Web، typecheck API/Web، lint محدوده و ۱۵ تست
...
## 2026-09-15 — بازگردانی طراحی خرید به مرجع تصویری — PC-B

محتوای عملیاتی `/purchases` بر اساس `procurement-optimized.html` به چیدمان کارت‌های رنگی، بنر مسیر خرید، نوار بخش‌ها و جدول پیگیری بازطراحی شد. شمارنده‌ها فقط وضعیت پرونده‌های واقعی صفحهٔ فعلی صف انتخاب‌شده را نشان می‌دهند؛ منوی تکراری، گزارش نمایشی و دادهٔ ساختگی مرجع بازنگشتند. فونت سراسری پروژه و API عملیاتی حفظ شده‌اند. typecheck، سه تست هدفمند با هفت مورد و build تولیدی Web با ۴۶ مسیر موفق‌اند؛ lint نیز موفق است. بدون Schema/Migration/Dependency یا تغییر داده.

## 2026-09-15 — فرم‌های بخش‌های خرید و ارجاع فاکتور به مالی — PC-B

فرم عملیاتی هر پرونده با عمل اصلی همان بخش باز می‌شود؛ ایجاد درخواست فقط در میزکار و درخواست‌ها دیده می‌شود. فرم تأمین‌کننده از مسیر مالک آن در اطلاعات پایه قابل دسترسی است. برای صف تأیید، استعلام، سفارش، دریافت، فاکتور و تأمین‌کننده نمونه‌های خواندنی بیشتری هنگام خالی بودن دادهٔ واقعی اضافه شد؛ این نمونه‌ها به API یا پایگاه داده فرستاده نمی‌شوند.

فاکتور عمومی تطبیق‌شده پس از ارجاع، با وضعیت PENDING از سرویس عمومی خرید و در محدوده شعب مجاز به کارتابل اصلی مالی نمایش داده می‌شود. خرید بلیت در همان کارتابل برقرار مانده است. این projection به‌تنهایی پرداخت، ثبت حسابداری یا پذیرش نهایی مالی ایجاد نمی‌کند و تغییر consumer در PR خرید باید توسط مالک مالی PC-A بازبینی شود. شش آزمون FinanceInbox و سی آزمون پایگاه داده Procurement، lint و typecheck API/Web و build تولیدی هر دو موفق‌اند. Schema، Migration، Dependency، Grant و Seed واقعی تغییر نکردند.

## 2026-09-15 — بازطراحی سکشن‌های داخلی خرید — PC-B

نمای تکراری داخلی خرید حذف و هر سکشن به فهرست مرحله‌ای، فیلترهای همان مرحله و کارت ورود به فرم اختصاصی تبدیل شد. درخواست‌ها از DraftForm، تأییدها از فرم تصمیم، تأمین‌کنندگان از فرم عمومی Master Data، و استعلام/سفارش/دریافت/فاکتور از OperationForm همان نوع استفاده می‌کنند. در دریافت و فاکتور، نوع سند با کنترل کوتاه داخل همان سکشن انتخاب می‌شود. متن‌های راهنمای طولانی و نوار سراسری اسناد از صفحات داخلی حذف شدند؛ نمونهٔ غیرفعال فیلدهای فرم، شکل هر سکشن را حتی هنگام خالی بودن داده واقعی نشان می‌دهد. URL بخش و پرونده برای Back/Forward حفظ شد.

فهرست API خرید پارامتر افزایشی `section` را با کنترل مجوز و شعبه می‌پذیرد و پرونده‌های دارای مرحله یا رکورد مرتبط را برمی‌گرداند؛ مشتری‌های قدیمی بدون این پارامتر همان پاسخ قبلی را می‌گیرند. سی آزمون PostgreSQL Procurement، lint و typecheck API/Web و build تولیدی هر دو با ۴۶ مسیر Web موفق‌اند. Schema/Migration/Dependency/Grant و دادهٔ واقعی تغییر نکرده‌اند.

## 2026-09-15 — رنگ‌بندی کارت‌های خرید — PC-B

کارت‌های شمارنده و مسیرهای میزکار، فهرست و فرم هفت سکشن داخلی و سرصفحهٔ سوابق با هاله، نشان و حاشیهٔ مرحله‌ای آبی/بنفش/فیروزه‌ای/کهربایی/سبز/رز مطابق الگوی HR و Master Data رنگ‌بندی شدند. رنگ متن و کنترل‌ها از توکن‌های مشترک روشن/تیرهٔ Web می‌آید. رفتار فرم، API، داده، Permission و Schema تغییر نکرد.

## 2026-09-15 — آزمون E2E فرم‌های خرید روی ۳۱۰۰ — PC-B

با نشست ذخیره‌شدهٔ حساب Nirvana در Gateway3100 ورود انجام شد. درخواست ساختگی
`PR-13018272-20a7-49ce-95bb-fcb2a6774496` با عنوان مشخص E2E، مبلغ
`1250000 IRR`، شعبه/درخواست‌کنندهٔ منتخب منابع انسانی، تاریخ میلادی 2026/10/15 و یک
ردیف کالا به‌صورت پیش‌نویس ذخیره شد؛ پس از بازخوانی از API و پایگاه محلی همان مقدارها
برگشتند. تأمین‌کنندهٔ ساختگی با کد `SUPPLIER_6RN2T6DL9O2X` از فرم اطلاعات پایه
ذخیره و در لیست واقعی دیده شد. انتخاب شعبه، کارمند، ارز، گرید ماه/سال شمسی و تقویم
میلادی، جست‌وجو و فیلتر وضعیت درخواست و Back مرورگر آزموده شدند. روی دادهٔ نمونهٔ
غیرذخیره‌شده، جست‌وجو فهرست واقعی را پرس‌وجو می‌کند و نمونه‌ها با ورود فیلتر پنهان
می‌شوند.

یک ایراد Web در بازکردن فوری پیش‌نویس مشاهده شد: تا تکمیل پرس‌وجوی گزینه‌های
ذخیره‌شده، نوع/دسته خرید و واحد سنجشِ دارای مقدار در API با placeholder نمایش داده
می‌شدند. مقدارهای همان پرونده به گزینه‌های اولیهٔ کنترل اضافه شد و آزمون بازخوانی بدون
cache برای آن ثبت شد. بخش‌های تأیید، استعلام، سفارش، دریافت و فاکتور به‌صورت UI و
گیت مجوز بررسی شدند؛ حساب فعلی مجوز عملیاتی تأیید/خریدار ندارد و سیاست تأیید هنوز
حساب/سقف تعیین‌شده ندارد. تنها پروندهٔ واقعی DRAFT است، پس ثبت عملیاتی این پنج مرحله
از این نشست E2E نشد. لینک کارتابل مالی به `/finance/requests` می‌رسد، ولی صف مالی
برای همین حساب مجاز نیست. ۱۷ آزمون Web خرید و ۵۴ آزمون API Procurement موفق بودند؛
۳۳ آزمون API وابسته به پایگاه اختصاصیِ فعال‌سازی‌نشده skip شدند. lint، typecheck،
Prettier و build Web با ۴۶ مسیر موفق‌اند. هیچ سفارش بیرونی، فاکتور عملیاتی یا پرداخت
ساخته نشد.

## 2026-09-15 — بازطراحی هدر فرم درخواست خرید — PC-B

سرصفحهٔ DraftForm و عنوان سه گروه اطلاعات درخواست، اقلام و خدمات، و اسناد/مرجع مبدأ
با نشان، رنگ‌های پروژه و مرزبندی روشن بازطراحی شدند. هدر هر ردیف قلم ماهیت کالا/خدمت
را نشان می‌دهد. کنترل «دوره ارائه خدمت» که برای کالا نیز بی‌جا نمایش داده می‌شد از
فرم حذف شد؛ فیلد `period` در قرارداد و مقدار رکوردهای قدیمی برای سازگاری حفظ می‌شود
و پیش‌نویس تازه آن را خالی می‌فرستد. فقط Web خرید و آزمون هدفمند آن تغییر کردند؛
API، Schema، Migration، Permission و داده تغییر نکردند. ۱۷ آزمون Web خرید،
lint، typecheck، Prettier و build کامل Web با ۴۶ مسیر موفق‌اند.

## 2026-09-15 — نمایش شناسه و کد خودکار خرید — PC-B

شمارهٔ داخلی درخواست در نخستین ثبت توسط API ساخته می‌شود و در فرم ویرایش نمایش
داده می‌شود؛ فرم تازه زمان تعیین آن را روشن می‌کند. فرم‌های سفارش و رسید شمارهٔ
درخواست مرجع و تولید خودکار شمارهٔ سند عملیاتی هنگام ثبت را نشان می‌دهند. شمارهٔ
فاکتور تأمین‌کننده و شناسهٔ مرجع بیرونی از سند مبدأ خوانده می‌شوند و شمارهٔ ساختگی
برای آنها تولید نمی‌شود. تولید کد و مالکیت داده تغییر نکرده است.
چهار آزمون هدفمند فرم، typecheck و build Web با ۴۶ مسیر موفق‌اند.

## 2026-09-15 — فیلتر تاریخ بخش‌های خرید — PC-B

میزکار و هفت سکشن داخلی خرید فیلتر بازهٔ تاریخ ثبت با تقویم مشترک آبی پروژه دارند؛
انتخاب شمسی/میلادی و شبکهٔ ماه/سال از همان DatePicker استفاده می‌کند. بازهٔ
`createdFrom/createdTo` روی فهرست درخواست‌ها در API و تاریخ ثبت تأمین‌کننده در
projection عمومی اطلاعات پایه اعمال می‌شود، بنابراین جست‌وجو، وضعیت و صفحه‌بندی با
تاریخ هماهنگ‌اند. تاریخ پایان به‌صورت UTC و با مرز انحصاری روز بعد فیلتر می‌شود.
فیلتر روی تاریخ ثبت پرونده است، نه تاریخ یک سفارش یا فاکتور داخلی آن پرونده.
پس از مرور در عرض حدود ۱۱۳۰ پیکسل، breakpoint چیدمان فیلترها از XL به LG منتقل
شد تا سه کنترل در یک ردیف قرار گیرند و در عرض کوچک به دو ردیف خوانا برگردند.
دو آزمون اعتبارسنجی API و پنج آزمون Web مربوط به سکشن/فرم موفق‌اند؛ ۳۰ آزمون
Procurement وابسته به پایگاه اختصاصی در اجرای معمول skip شدند. typecheck و build
API/Web (۴۶ مسیر) موفق‌اند.

## 2026-09-15 — مسیر پویای سکشن‌های خرید — PC-B

breadcrumb پوستهٔ اصلی از hook موجود `usePageBreadcrumbs` برای `/purchases`
عنوان بخش فعال را نمایش می‌دهد؛ هنگام بازشدن پرونده شمارهٔ آن نیز به مسیر افزوده
می‌شود. انتخاب والد، فهرست یا میزکار خرید را باز می‌کند و تغییر URL/Back مرورگر
همان مسیر را بازسازی می‌کند. مسیرهای تکراری محلی از میزکار و سکشن‌ها حذف شدند.
فایل پوستهٔ مشترک، API، Migration و داده تغییر نکردند.
پنج آزمون هدفمند Web، typecheck و build تولیدی با ۴۶ مسیر موفق‌اند.
## Procurement HR directory and standalone suppliers — 2026-09-16

- فرم درخواست خرید اکنون شعبهٔ کارمند جاری را از projection عمومی HR و، در نبود اتصال حساب کاربری به پروندهٔ کارمند، اولین شعبهٔ مجاز دارای کارمند فعال را به‌صورت پیش‌فرض انتخاب می‌کند؛ در نتیجه در حساب‌های چندشعبه‌ای نیز فهرست کارکنان فعال و واحدهای سازمانی ثبت‌شده در HR بدون انتخاب دستی اولیه بارگذاری می‌شوند.
- فرم ایجاد تأمین‌کننده مستقل شد: نام تأمین‌کننده در خود کاتالوگ خرید ثبت می‌شود و فرم دیگر Organization یا مخاطب Organization را درخواست نمی‌کند. API نیز هنگام ایجاد رکورد تازه هر لینک Organization/Contact ارسالی از کلاینت قدیمی را خنثی می‌کند؛ لینک رکوردهای قدیمی فقط برای سازگاری خواندن حفظ شده است.
- اعتبارسنجی نهایی: ۲۳ تست هدفمند Web، ۱۰ تست هدفمند API، lint، typecheck و build تولیدی برنامه‌های Web/API موفق‌اند.

## Procurement invoice and Finance handoff UI — 2026-09-16

- دکمهٔ «باز کردن فرم» برای پرونده‌های خواندنی سکشن فاکتور اکنون فرم قابل‌کار همان پرونده را باز می‌کند. فرم شماره و سفارش مرجع، تاریخ فاکتور و سررسید با تقویم مشترک، مبلغ و ارز را می‌گیرد و جریان «ثبت فاکتور → تطبیق فاکتور و تحویل → ارجاع فاکتور به مالی» را مرحله‌ای اجرا می‌کند؛ وضعیت هر نمونه فقط در `sessionStorage` همان تب نگه‌داری می‌شود و هیچ تعهد، پرداخت یا سند حسابداری واقعی نمی‌سازد.
- جریان واقعی تغییری نکرده و همچنان پس از `INVOICE` و `MATCH_INVOICE`، فرمان `SUBMIT_FINANCE` یک projection نسخه‌دار `procurement.finance-source.v1` می‌سازد که `FinanceInboxService` از public service خرید می‌خواند. پایگاه فعال فعلاً ۱۱ درخواست و صفر سفارش/فاکتور/ارجاع واقعی دارد؛ بنابراین داده‌های صفحهٔ فاکتور نمونهٔ خواندنی‌اند.
- کاتالوگ runtime محلی نقش `finance_staff` فقط مجوزهای اسناد را داشت. هفت مجوز تعریف‌شدهٔ رسمی این نقش از Seed به‌صورت افزایشی sync شد، نقش به حساب `nirvana` متصل و Audit محلی ثبت شد. کارتابل `/finance/requests` اکنون به‌جای ۴۰۳ پاسخ موفق و حالت خالی واقعی نشان می‌دهد. هیچ مجوز قبلی حذف یا به سایر کاربران واگذار نشد.
- دو آزمون هدفمند Web، lint، typecheck و build تولیدی ۴۶ مسیر Web موفق‌اند.

## Procurement reusable form choices — 2026-09-16

- گزینه‌های تازهٔ «نوع خرید»، «دسته خرید» و «واحد سنجش» همراه پیش‌نویس درخواست
  ذخیره می‌شوند و پاسخ ذخیره بلافاصله وارد cache فهرست گزینه‌های قابل انتخاب می‌شود؛
  بنابراین بازکردن فرم بعدی دیگر تا پایان stale time، فهرست قدیمی را نشان نمی‌دهد.
- متن راهنما کنار ورودی تازه روشن می‌کند که افزودن مورد با دکمهٔ «ذخیره پیش‌نویس»
  انجام می‌شود. تست هدفمند شش‌گانه، lint، typecheck و build تولیدی ۴۶ مسیر Web
  موفق‌اند؛ API، Schema، Migration و دادهٔ عملیاتی تغییر نکردند.

## Procurement request origin form cleanup — 2026-09-16

- انتخاب «نوع منشأ درخواست» و فیلدهای ارجاع رزرواسیون از فرم درخواست خرید حذف
  شدند. پیوست‌ها و یادداشت تکمیلی با عنوان مستقل «پیوست‌ها و یادداشت‌ها» باقی
  ماندند. مقدار origin داخلی برای سازگاری داده و قراردادهای موجود حفظ شده و API،
  Schema و Migration تغییر نکرده‌اند.

## Dashboard KPI compact money fit — 2026-09-16

- مبلغ‌های فشردهٔ KPI در Dashboard اکنون با توجه به طول نمایشی و عرض واقعی هر
  کارت (واحد CSS `cqw`) اندازهٔ فونت واکنش‌گرا می‌گیرند؛ بنابراین کارت عریض متن
  بزرگ‌تر را حفظ می‌کند و کارت کوچک فقط هنگام نیاز کوچک می‌شود. نماد و مبلغ همچنان
  یک واحد بدون شکست‌اند، تگ رشد حفظ شده و مقدار دقیق در Tooltip و پنل تعریف قابل
  ممیزی KPI در دسترس است. lint و TypeScript Web موفق‌اند؛ API، Reports، دیتابیس و
  Migration تغییر نکرده‌اند.

## Dashboard sidebar information architecture — 2026-09-16

- نام صفحه‌های Sidebar با KPIها و تحلیل‌های خود بازبینی شد و بدون تغییر شناسه یا
  URL، حفظ شد. سربرگ ناوبری اکنون «صفحه‌های داشبورد» است و Tab فیلتر با عنوان
  «فیلترهای این صفحه» نام صفحهٔ فعال را در Heading نمایش می‌دهد. نام‌های عمومی
  فیلترها به «کانال فروش»، «کارشناس مسئول»، «خدمت سفر»، «آژانس همکار»،
  «تأمین‌کننده خدمت» و «واحد پول» شفاف شدند.
- برچسب «وضعیت» اکنون با scope واقعی صفحه نمایش می‌یابد؛ مانند وضعیت پرداخت، رزرو
  و صدور، خرید، مالی، لید و مشتری یا درخواست پشتیبانی. در نماهای Customer و
  Marketing «کانال جذب»، در نماهای کارکنان «کارمند»/«کارشناس فروش»/«کارشناس CRM»
  و در پشتیبانی «کارشناس پشتیبانی» نشان داده می‌شود. دو فیلتر نامرتبط خدمت سفر نیز
  از صفحات منابع انسانی و کیفیت سوابق پرسنلی حذف شدند. API، Reports، دیتابیس و
  Migration تغییر نکرده‌اند.

## Dashboard English numeric presentation — 2026-09-16

- تمام خروجی‌های عددی Dashboard در KPI Cardها، تگ رشد، Tooltip، محور و عنوان‌های
  نمودار، Legend، جدول‌های تحلیلی و خلاصهٔ دسترس‌پذیر اکنون با ارقام لاتین
  `en-US` نمایش می‌یابند. نمایش فشردهٔ مبلغ‌ها و محورهای نمودار از `K/M/B` استفاده
  می‌کند؛ مثال: `﷼ 858.8M` و `$ 44.1M`.
- این تغییر صرفاً لایهٔ ارائه است: نماد هر ارز، مبلغ دقیق قابل بازیابی، سری‌های
  مستقل چندارزی، محاسبات و دادهٔ منبع دست‌نخورده‌اند. API، Reports، دیتابیس و
  Migration تغییر نکرده‌اند.

## Dashboard sticky page navigation — 2026-09-16

- فقط پنل «صفحه‌های داشبورد» در Sidebar راست، از نمایش دسکتاپ به بعد با فاصلهٔ
  امن زیر نوار بالایی ثابت است و حداکثر ارتفاع آن به viewport محدود شده است. بنابراین
  در اسکرول طولانی پنهان نمی‌شود و فهرست بلند داخل خود اسکرول قابل‌دسترسی دارد.
  پنل «فیلترهای این صفحه» نیز در دسکتاپ همین رفتار را دارد؛ تجربهٔ موبایل همچنان با
  سند حرکت می‌کند.

## Dashboard centered page headers — 2026-09-16

- سربرگ تمام صفحه‌های Dashboard اکنون مرکزچین است و آیکون متناسب با صفحه، عنوان
  فارسی، نام انگلیسی و توضیح همان صفحه را از Registry نشان می‌دهد. پس‌زمینهٔ
  گرادیانی آبی/فیروزه‌ای ملایم و glow محدود، با پالت Dashboard و حالت تاریک سازگار
  است؛ چیدمان فشرده می‌ماند و فضای خالی غیرضروری ایجاد نمی‌کند.

## Dashboard page-specific header icons — 2026-09-16

- نگاشت آیکون سربرگ برای همهٔ صفحه‌های اصلی و زیرصفحه‌های Dashboard کامل شد؛ بنابراین
  آیکون fallback نمای مدیریتی دیگر روی زیرصفحه‌ها نمایش داده نمی‌شود. آیکون‌ها با
  موضوع صفحه هم‌خوان‌اند؛ نمونه‌ها: Plane/Ticket برای عملیات سفر و بلیت، Hotel برای
  تور و هتل، Banknote برای وصول، ShieldAlert برای ریسک مالی و PhoneCall برای فعالیت
  CRM یا پشتیبانی.

## Dashboard reference donut layout — 2026-09-16

- همهٔ Donut Chartهای Dashboard برای دسته‌های کم‌تعداد با حلقهٔ بزرگ، طیف رتبه‌ای
  آبی و برچسب‌های بیرونی دارای خط راهنما بازطراحی شدند. هر برچسب فقط نام دسته، مقدار
  فشرده و درصد واقعی همان visual را نشان می‌دهد؛ مجموع واقعی نیز در مرکز حلقه باقی
  می‌ماند. بنابراین «سود»، «هزینه» یا هر مقدار غیرموجود در Projection ساخته نمی‌شود.
- قاعدهٔ موجود حفظ شد: visualهای donut با بیش از شش دسته به Bar تبدیل می‌شوند تا
  برچسب‌ها خوانا و تصمیم‌پذیر باقی بمانند. API، Reports، دیتابیس، Migration و منطق
  محاسبهٔ داده تغییری نکرده‌اند.

## Dashboard donut readability — 2026-09-16

- قاب Donutها به حداکثر ۱۹rem و ارتفاع فشردهٔ ۶۰/۶۴ کاهش یافت تا فضای افقی
  غیرضروری اشغال نشود. قطر حلقه کوچک‌تر شد و متن نام دسته، مقدار فشرده و درصد در
  برچسب بیرونی بزرگ‌تر شد؛ مجموع مرکزی نیز در اندازه‌ای نگه داشته شد که با مقدار
  واقعی طولانی از حلقه بیرون نزند.

## Dashboard donut label legibility — 2026-09-16

- بررسی خروجی واقعی نشان داد قاب فشردهٔ قبلی فضای امن کافی برای برچسب‌های بیرونی
  نداشت. بوم Donut اکنون نسبت ۴۴۰×۳۲۰ و حلقهٔ واکنش‌گرای ۱۶۰/۱۹۲px دارد؛ خطوط
  راهنما بلندتر و پرکنتراست‌تر شدند و نام دسته و مقدار/درصد واقعی در دو خط با
...
- فرم ایجاد تأمین‌کننده مستقل اکنون «نشانی» و «شماره تماس اصلی» دارد. کد داخلی
  تأمین‌کننده در Backend به‌صورت یکتا تولید می‌شود و فیلد دستی Provider/API از فرم
  حذف شد. تلفن با سرویس رمزنگاری تماس ذخیره و فقط مقدار ماسک‌شده در projection
  عمومی بازگردانده می‌شود؛ داده رمزنگاری‌شده از پاسخ و Audit حذف می‌شود.
- Migration افزایشی `20260916121500_supplier_direct_contact` پس از backup روی
  پایگاه محلی اعمال شد. Schema/Prisma validate و generate، یک تست Migration، ۱۱
  تست API و پنج تست Web هدفمند موفق‌اند.

- فرم تو در توی «ایجاد خدمت مرجع» در انتخاب خدمات تأمین‌کننده دیگر نام انگلیسی
  دریافت نمی‌کند. کد همچنان خودکار تولید می‌شود و نام فارسی ورودی نام خدمت است؛
  داده‌های قدیمی و قرارداد Backend سازگار باقی مانده‌اند.
