...
پس از جایگزینی اجرای میزکار با نسخه تازه آژانس‌ها، میزکار مجدداً بر پایه `56d5d48` اضافه می‌شود تا فیلترهای جدید تاریخ حفظ شوند. با فعال‌بودن محیط نمونه، مسیر منوی فعلی `/tasks` مستقیماً به `/workbench/demo` هدایت می‌شود. بدون تغییر API، داده، Schema، Dependency یا منوی مشترک. نتیجه آزمون و هماهنگی اجرا در [WORKBENCH-004](tasks/WORKBENCH-004-MENU-CURRENT-RUNTIME.md) ثبت می‌شود.
