## 2026-09-16 — RESERVATION-ACTION-DENSITY-0916 — PC-A — READY_FOR_REVIEW

پنل عملیات قرارداد در پایین جدول رزواسیون نگه داشته شد، اما دیگر در عرض و ارتفاع صفحه کش نمی‌آید: خلاصهٔ قرارداد و سه گروه عملیات در دسکتاپ کنار هم هستند، دکمه‌ها دو ستونه و ۳۲ پیکسلی‌اند و چیدمان در نمایش کوچک‌تر به‌صورت واکنش‌گرا بازچینش می‌شود. فاصلهٔ پنل و جدول کم و جدول به ارتفاع نزدیک هفت ردیف محدود است؛ فقط داخل جدول اسکرول می‌شود و صفحه برای دیدن عملیات اصلی نیاز به اسکرول اضافی ندارد. ۲۱ تست هدفمند رزواسیون، lint، typecheck و build تولیدی Web با ۴۶ مسیر موفق‌اند. Migration، API، دادهٔ عملیاتی و Runtime مشترک تغییری نکرده‌اند.
