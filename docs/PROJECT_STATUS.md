...
سه تب گزارش، Audit و خروجی به سوابق واقعی B2B، اطلاعات سازمان و اسناد متصل شدند. فیلتر شعبه/بخش/منبع/نتیجه و تاریخ شمسی، جزئیات انجام‌دهنده و فیلدهای تغییرکرده و خروجی تمام صفحات Excel پیاده شد. تاریخچه حذف نماینده/نرخ حفظ و حذف دائمی سند برای حفظ لاگ اصلاح شد؛ فایل و نسخه حذف می‌شوند و نشانگر حداقلی غیرقابل‌دسترسی باقی می‌ماند. تست PostgreSQL جداسازی سازمان/شعبه، صفحه‌بندی، تاریخ و حذف سند موفق است. روی داده موجود افق سفر ۲۲ رویداد از هر سه منبع بازخوانی شد. Build و فعال‌سازی نهایی در حال انجام است؛ [جزئیات](tasks/B2B-DOSSIER-REPORTS-001.md).
