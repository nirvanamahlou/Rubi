...
- مبلغ‌های فشردهٔ KPI در Dashboard اکنون با توجه به طول نمایشی خود اندازهٔ فونت
  واکنش‌گرا می‌گیرند: مقدارهای معمول `text-lg`، مقدارهای بلند `text-sm` و موردهای
  بسیار بلند `text-xs`. نماد و مبلغ همچنان یک واحد بدون شکست‌اند، تگ رشد حفظ شده
  و مقدار دقیق در Tooltip و پنل تعریف قابل ممیزی KPI در دسترس است. lint و
  TypeScript Web موفق‌اند؛ API، Reports، دیتابیس و Migration تغییر نکرده‌اند.
