...
اعتبارسنجی نهایی WORKBENCH-011: تعداد ۵۸ تست هدفمند، lint، typecheck و build تولیدی موفق؛ خانه و کارتابل با حساب واردشده و مجوز واقعی بررسی شدند. نسخه29cfe14 روی Web3100 فعال است؛ API4190 بدون تغییر و سالم است. PR174 پیش‌نویس و بدون merge؛ وابستگی‌های پایدار همچنان مسدودند.
...
فعال‌سازی API هنوز انجام نشده است: بررسی خودکار تعویض PID15024 را دوبار (بار دوم پس از تأیید صریح کاربر) پیش از اجرا رد کرد. API قبلی محفوظ است. فرم با بررسی قابلیت سرویس، تا انتشار API جدید هیچ رمزی نمی‌پذیرد. جمع آزمون‌ها با پوشش این وضعیت ۸۱ مورد است؛ PR171 پیش‌نویس، بدون Merge.
...
نسخهfb7a6dd روی۳۱۰۰/PID23768 فعال است. ۴۱ تست، lint، typecheck/build و آزمون مرورگر افزودن/برداشتن ستاره در دو تب موفق بود. دادهٔ فایل تغییر نکرد و ستارهٔ آزمایشی به وضعیت اولیه برگشت. PR167 پیش‌نویس، بدون Merge.

## WORKBENCH-007 — فرم یادداشت جدید

دکمه «یادداشت جدید» در سربرگ میزکار و تب یادداشت‌ها، فرم مشترک روبی را با عنوان، متن، شمارنده طول و تأیید بستن متن ذخیره‌نشده باز می‌کند. ذخیره دائمی هنوز تکمیل نشده است: آخرین شاخه مالی و PR153 قفل Migration نزد رزرواسیون را تأیید می‌کنند و هماهنگ‌کننده runtime اختیار واگذاری آن را ندارد. فرم این محدودیت را پیش از ورود متن نشان می‌دهد و ثبت غیرفعال است؛ هیچ داده جعلی، Browser storage یا استفاده از جدول ماژول دیگر اضافه نشده است. ادامه persistence به handoff مالک Migration و قرارداد خصوصی یادداشت وابسته است.
