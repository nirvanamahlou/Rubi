...
## صدور بلیط و Gate تحویل مالی — 2026-09-12 — فعال در لوکال

بلیط از زمان ورود قرارداد به رزرواسیون صادرشده محسوب می‌شود و رزرواسیون می‌تواند PDF یک مسافر یا همه مسافران را دریافت کند. خروجی صادرشده فقط سربرگ، مشخصات قرارداد/مسافر و اطلاعات پرواز را نگه می‌دارد؛ متن‌های Draft، Notice و توضیحات آزمایشی حذف و وضعیت `ISSUED` درج شد. مسیر دانلود فروش اکنون از endpoint مالکیت Sales و تأیید تحویل مدارک مالی عبور می‌کند؛ MANIFEST نیز پیش از خواندن اطلاعات هویتی مسافر همین تأیید را الزام می‌کند. ۱۳ تست Web، ۸ تست API، lint، typecheck، build تولیدی دو برنامه و بازبینی تصویری PDF A4 موفق‌اند. بدون Migration، تغییر IAM یا داده عملیاتی.

## تکمیل دادهٔ تست سفر خارجی قراردادهای قدیمی — 2026-09-12 — فعال در لوکال

پنج قرارداد لوکال `SC-2026-000001` تا `SC-2026-000005` برای آزمون فروش و MANIFEST با اطلاعات کاملاً ساختگی تکمیل شدند. فقط فیلدهای خالی چهار پروندهٔ مسافر یکتا شامل نام لاتین پاسپورت، جنسیت، ملیت، کشور صادرکننده و محل تولد پر شد؛ دو پروندهٔ فاقد پاسپورت نیز شمارهٔ رمزنگاری‌شده و تاریخ انقضای تستی گرفتند. نسخه هر پرونده افزایش و Audit ثبت شد. کنترل نهایی هر پنج قرارداد را بدون فیلد اجباری خالی نشان می‌دهد؛ هیچ داده واقعی یا تغییر کد/Schema انجام نشد.
