...
## HR-013 — ارجاع و پاسخ بین منابع انسانی و بخش‌های سامانه — آماده بازبینی

- Commit پیاده‌سازی `93c0a7e` به origin ارسال شد؛ PR پیش‌نویس #139 به develop باز است. Merge و فعال‌سازی محلی انجام نشده‌اند.
...
- ۱۰۱ تست HR Web، ۷۵ تست HR API، ۲۲ تست واقعی PostgreSQL، ۶۳ تست Contracts و ۷۳ تست بسته Database پاس شدند. lint/typecheck چهار بسته و build نهایی API/Web با ۴۰ مسیر موفق‌اند. آزمون اولیه یک fixture پوشیده را اصلاح کرد؛ یک timeout آزمون قدیمی هنگام build هم با اجرای ترتیبی مجدد رفع شد.
- این تحویل لایه ارجاع/پاسخ است؛ پرداخت واقعی، صدور سفر، قطع دسترسی، تکمیل خرید و اتصال دستگاه اجرا نشده‌اند. بدون Migration، تغییر Dependency، تغییر کاربر واقعی یا جایگزینی اجرای ۳۱۰۰. جزئیات استقرار هماهنگ و موارد باقی‌مانده: `docs/tasks/HR-013-CONNECTIONS.md`.
