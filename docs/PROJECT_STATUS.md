...
## WORKBENCH-003 — نسخه کامل با میزکار، آماده انتقال به ۳۱۰۰

نسخه مستقل از source اجرای تأییدشده `7ada379` ساخته شد تا مسیر محدود `/workbench/demo` و لینک فعال‌شونده با تنظیم محیط در صفحه فعلی میز کار (`/tasks`) اضافه شود. مالک Runtime انتقال فقط Web3100 را پس از ساخت و بررسی روی پورت آزاد تحویل داده است. API4190، داده‌ها، فایل‌های اسناد و کار فعال B2B دست‌نخورده می‌مانند. جزئیات و نتیجه نهایی: [WORKBENCH-003](tasks/WORKBENCH-003-RUNTIME-3100.md).
