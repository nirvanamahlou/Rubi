...
فرم «درخواست خرید جدید» اکنون بدون ثبت یا انتخاب تأمین‌کننده قابل تکمیل و ثبت است؛ تأمین‌کننده در درخواست اولیه اختیاری است و برای مرحلهٔ استعلام/سفارش تعیین می‌شود. توضیح این قاعده داخل فرم اضافه شد تا مسیر ایجاد درخواست روشن باشد. قواعد دامنه نیز تست می‌کنند که ارسال درخواست بدون `supplierId` یا `supplierName` معتبر است؛ الزام تأمین‌کننده در استعلام، سفارش و فاکتور تغییری نکرده است. تست هدفمند، lint و typecheck Web/API موفق‌اند. Branch: `codex/pc-b-procurement-request-no-supplier-0921`.
...
تب و دکمهٔ تکراری «تنظیمات بخش‌ها» از مدیریت سیستم حذف شدند. ناوبری بالای صفحه اکنون هفت گروه اصلی «فضای کار»، «فروش و ارتباط با مشتری»، «رزرواسیون و تامین سفر»، «مالی»، «سرمایه انسانی»، «اسناد و گزارش» و «تنظیمات شرکت» را نشان می‌دهد و هر انتخاب فقط کارت‌های تنظیمات همان حوزه را نمایش می‌دهد. عملیات ویرایش نسخه‌دار و دکمه‌های بررسی/تاریخچهٔ تغییرات حفظ شده‌اند. ۴ تست اختصاصی، ESLint، typecheck و build تولیدی Web موفق‌اند و مرورگر احرازشده روی localhost:3100 نتیجه را تأیید کرد. یک تست قراردادی قدیمیِ مستقل برای لینک‌های IAM/Legal Entity پیش از این تغییر با source حاضر ناسازگار بود؛ خارج از محدوده باقی ماند. Migration، API، داده، مجوز و وابستگی‌ها تغییری نکردند.

## 2026-09-19 — LOGIN-NOORA-CLOUD-OUTLINE-0919 — PC-B — READY_FOR_REVIEW

ابر مه‌آلود صفحهٔ ورود با ابر خطی آبیِ مرجع جایگزین شد؛ سطح داخلی نیمه‌شفاف است و `NOORA` در مرکز آن دیده می‌شود. پس از توقف هواپیما ظاهر می‌شود و برای کاربران با کاهش حرکت، بدون انیمیشن نمایش دارد. ۲ تست هدفمند، typecheck و build تولیدی Web موفق‌اند و نسخهٔ جدید روی localhost:3100 بازراه‌اندازی شد. منطق ورود، API، Migration و دادهٔ عملیاتی تغییری نکرده‌اند.
