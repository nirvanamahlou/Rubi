...
## NAVIGATION-001-PROCUREMENT-HR-GROUP — PC-B — READY_FOR_REVIEW

مسیر موجود «خرید و تأمین» در منوی اصلی از گروه «مالی» به گروه «سرمایه انسانی» منتقل شد.
URL، عنوان، دسترسی‌ها، API و منطق مالی تغییر نکرده‌اند. آزمون قرارداد ناوبری جایگاه
...
## PROCUREMENT-001 — PC-B — آماده بررسی در Draft PR

Workspace واقعی `/purchases` با درخواست، تأیید نسخه‌دار، پیشنهاد، سفارش، دریافت،
پذیرش، اصلاح جبرانی، فاکتور و تطبیق پیاده شده است. گزارش و صف‌ها از داده scoped
واقعی استفاده می‌کنند؛ خروجی صف‌بندی‌شده به Documents و اعلان به Notifications
متصل است. سیاست خرید مصوب، مصرف‌کننده Finance/Tasks و ارجاع Reservations هنوز
Fail-closed هستند. در محیط آزمایشی ایزوله PostgreSQL18، هر ۶۴ Migration اعمال و
Seed نهایی دو بار بدون تغییر شمارش اجرا شده؛ ۱۴ تست یکپارچگی و ۸۲ تست هدفمند
API/دامنه/خروجی موفق‌اند. تست کامل API با ۱۳۱۳ و Web با ۱۳۵۳ مورد، lint،
typecheck و ساخت API/Web/Worker/Packageها موفق شدند. P95 فهرست در آزمون
۱۰۰ هزار پرونده و ۵۰ فراخوان هم‌زمان حدود ۱٫۱۲ ثانیه بود؛ این اندازه‌گیری شامل
HTTP و مرورگر نیست. خروجی واقعی PDF/Excel از Documents پس از اسکن CLEAN دریافت شد.
Merge یا استقرار انجام نشده و انتشار به سیاست تجاری و قراردادهای مالک وابسته است.
جزئیات در [PROCUREMENT-001](tasks/PROCUREMENT-001.md) است. کنترل‌های تقویم خرید و تأمین با الگوی مرجع روبی همسان شد: گرید ۳×۴ ماه و سال شمسی، گرید روز با روزهای مجاور کم‌رنگ، تب میلادی با برچسب‌های انگلیسی و فیلترهای «از تاریخ» و «تا تاریخ». مقدار تاریخ انتخاب‌شده اکنون در همان فیلتر نمایش داده می‌شود و پاک‌کردن، هر دو کنترل را به حالت اولیه برمی‌گرداند. کلیک روی فضای بیرون تقویم نیز آن را می‌بندد، درحالی‌که تعامل‌های داخل تقویم فعال می‌مانند. در نوار فیلتر فهرست درخواست‌ها، عنوان‌های «جست‌وجو»، «وضعیت درخواست» و «واحد درخواست‌کننده» نمایش داده می‌شوند و انتخاب‌گر داخلی حالت نمایشی حذف شده است. عنوان فیلد مبلغ فرم خرید نیز به «مبلغ» کوتاه شد. همه فرم‌های ماکاپ با کلیک روی فضای بیرون بسته می‌شوند و کلیک داخل فرم آن را باز نگه می‌دارد. نوار داخلی تکراری شامل داده‌های آزمایشی و اعلان نیز از فضای خرید حذف شد. اقدام بالای هر سکشن اکنون فرم مرتبط همان بخش را باز می‌کند؛ سکشن‌های تصمیم و ارجاع سفر که ایجاد رکورد مستقل ندارند، دکمه ایجاد عمومی نمایش نمی‌دهند. عنوان فیلترهای درخواست، تأمین‌کننده، پیشنهاد، سفارش، دریافت و فاکتور نیز متناسب با همان سکشن نمایش داده می‌شود. ناوبری سکشن‌ها و جزئیات پرونده اکنون برای هر حرکت History مستقل می‌سازد و رویداد `popstate`، Back و Forward مرورگر را به وضعیت متناظر خرید برمی‌گرداند. فیلترهای تمام سکشن‌ها بازطراحی شدند: کنترل‌های اختصاصی داده هر بخش، فیلترهای رابطه‌ای تأمین‌کننده/درخواست/سفارش، وضعیت تطبیق فاکتور، بازه تاریخ اختیاری و پاک‌سازی یکپارچه در چیدمان فشرده و واکنش‌گرای همسان با تم روبی قرار گرفتند.
متن توضیحی ماکاپ از پایین فضای خرید حذف شد.
آدرس اصلی `/purchases` اکنون سکشن فعال و پرونده بازشده را در Query String نگه می‌دارد؛ ارتباط دوطرفه والد و iframe باعث می‌شود انتخاب سکشن و Back/Forward مرورگر با محتوای خرید همگام بمانند.
