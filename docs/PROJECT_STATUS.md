...
## 2026-09-16 — RESERVATION-PASSENGER-IDENTITY-EDIT-0916 — PC-A — READY_FOR_REVIEW

ویرایش مشخصات هویتی پروندهٔ اصلی مسافر از رزواسیون در حال پیاده‌سازی است. حیطه فقط CustomerService از طریق قرارداد عمومی Reservations و جدول مسافران است؛ snapshot قرارداد، مدارک صادرشده، Migration، دادهٔ عملیاتی و Runtime مشترک تغییر نمی‌کنند.

نتیجه: جدول اسامی مسافران اکنون مشخصات اصلی فروش، از جمله نام لاتین، جنسیت، تاریخ تولد، کد ملی و پاسپورت را در همان پروندهٔ مشتری اصلاح می‌کند. نمایش و ذخیرهٔ مشخصات حساس به customers.sensitive.read و customers.update نیاز دارد؛ نام همچنان با مجوز ویرایش عادی قابل اصلاح است. lint فایل‌های متاثر موفق است. تست هدفمند API در worktree تازه به‌دلیل نبود خروجی build بستهٔ @nora/database پیش از اجرای تست متوقف شد؛ این خطا خارج از تغییر است.
