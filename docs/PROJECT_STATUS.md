...
## 2026-09-22 — رفع ایرادهای QA خرید و تأمین — READY_FOR_REVIEW

ذخیره و انتشار درخواست خرید اکنون در یک Mutation و تراکنش انجام می‌شود؛ اعتبارسنجی کامل
پیش از نوشتن و قفل کلیک تکراری مانع ایجاد پیش‌نویس پنهان یا ردیف تکراری است. پیام خطا نام
عمل را دقیق بیان می‌کند و فوکوس را به نخستین کنترل نامعتبر می‌برد. fallback داده‌های نمایشی
در جست‌وجو، وضعیت و تاریخ از دست نمی‌رود، نام تأمین‌کننده و رویدادهای تاریخچه با برچسب
انسانی نمایش داده می‌شوند و شناسه خام رکوردهای داخلی از رابط حذف شده است. بازیابی نشست نیز
هویت سربرگ را بدون reload تازه می‌کند؛ Skip Link، الزامات دسترس‌پذیر و سقف ۳۰۰ نویسه عنوان
اعمال شده‌اند.

۲۹ تست متمرکز Web و بازاجرای ۹ تست فرم، lint محدوده، typecheck API/Web و build تولیدی هر
دو برنامه موفق‌اند؛ مجموعه کامل API پیش‌تر در همین QA با ۱۵۵۷ تست موفق و ۱۷۰ تست DB-gated
ردشده توسط guard اجرا شد. بازآزمایی عملی روی `localhost:3100` با حساب ساختگی، انتشار اتمیک
بدون تأمین‌کننده، HR dropdown، تقویم، URL و Back، RTL، جایگاه منوی خرید زیر سرمایه انسانی و
کنسول بدون خطا را تأیید کرد. تست یکپارچهٔ نیازمند دیتابیس ایزوله و viewport واقعی موبایل/تبلت
اجرا نشدند؛ هیچ پرداخت، پیام بیرونی، حذف داده، Migration یا تغییر مجوز عمومی انجام نشد.
