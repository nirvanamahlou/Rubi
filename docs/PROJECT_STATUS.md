...
## HR-006 — بازطراحی منابع انسانی با تم روبی — آماده بررسی

- `PC-B` روی `codex/pc-b-hr-rubi-redesign` و پایه HR-005: همان ۱۴ مقصد و زیرگروه‌های فعلی حفظ شدند. داشبورد، فهرست‌ها، فیلتر و عملیات ردیف، پرونده کارمند، فرم‌ها، چارت، تقویم شیفت، گزارش و اعلان‌ها با اجزای مشترک و تم روشن/تیره روبی یکدست شدند؛ کارت‌های اصلی الگوی اطلاعات پایه را حفظ می‌کنند.
- بارگذاری فرم و ورود اکسل مرحله‌ای است؛ بخش گزارش فقط هنگام بازشدن رندر می‌شود. توابع داده نمایشی از صفحه‌های سنگین جدا شدند و تبدیل تاریخ تقویم تکراری کش می‌شود. ردیف سکشن قبلی هنگام تغییر تب نشان داده نمی‌شود و داشبورد پیش از دریافت داده، شمارش صفر ساختگی نمایش نمی‌دهد.
- lint، typecheck، ۸۵ تست منابع انسانی و Production Build موفق‌اند. مرورگر واقعی: ۱۴ مقصد، فیلتر، ویرایش/تأیید حذف، خروجی Excel/PDF، پرونده و عکس، فرم متقاضی، تقویم سازمانی و موبایل، اعلان‌های قابل اسکرول، چهار عرض و حالت تیره بررسی شدند. هیچ تغییر API، Schema/Migration، Dependency یا داده کسب‌وکار انجام نشد.
- تحویل عملیاتی با Task «PC-B Uniting»: پورت‌های ۳۱۰۰/۴۰۰۰ برای نسخه یکپارچه مورد تأیید مالک آزاد شدند. پیش‌نمایش جداگانه HR-006 روی `http://127.0.0.1:3188/hr` و API روی ۴۱۸۸ است؛ نسخه یکپارچه و کپی دیتابیس آن دست‌نخورده ماندند. این شاخه بدون Merge برای Review تحویل می‌شود؛ جزئیات در `docs/tasks/HR-006.md`.

## HR-005 — منابع انسانی یکپارچه و پایدار — آماده بررسی

- اصلاح طراحی به درخواست مالک: کارت‌های صفحه اصلی HR با الگوی اطلاعات پایه یکسان شدند؛ چهار ستون دسکتاپ، دو ستون تبلت و یک ستون موبایل، رنگ و آیکون، برچسب زیرمجموعه‌ها و حالت‌های hover/focus. مسیرها و شمارش داده واقعی حفظ شدند. lint، typecheck، ۱۲ تست مرتبط و Build موفق‌اند؛ بررسی بصری کامپوننت با CSS خروجی در چهار عرض و حالت تیره بدون سرریز انجام شد.
- `PC-B` روی `codex/pc-b-hr-unified-workflows`: مسیرهای منابع انسانی به API احراز‌شده و ذخیره‌سازی PostgreSQL متصل شدند؛ نسخه قدیمی `/human-resources` به `/hr` هدایت می‌شود. تغییرات کارمند/واحد/درخواست، نسخه، مجوز شعبه، حذف منطقی و تاریخچه دارند و اعلان‌ها در زنگوله ماندگارند.
- کارکرد و زمان، شیفت‌ها، قراردادها، پرونده جذب، تغییرات شغلی/خروج، مأموریت/هزینه، ارزیابی/آموزش، تجهیزات/خودرو و پرونده شخصی کارمند یکپارچه شدند. محاسبه حضور، اصلاح تردد، بستن دوره، مانده مرخصی و تغییر شغلی مؤثر بر پرونده با قواعد سرور اجرا می‌شوند.
- فرم‌ها و انتخاب‌گرها از داده مجاز شرکت استفاده می‌کنند؛ اکسل ورودی دارای پیش‌نمایش/تشخیص تکرار است و خروجی اکسل و PDF از ردیف‌های واقعی و فیلترشده ساخته می‌شوند. PDF قرارداد با لوگوی شرکت، نسخه، متن فارسی قابل جست‌وجو و فرم ثبت نسخه امضاشده ارائه می‌شود.
- اعتبارسنجی: ۷۸ تست API منابع انسانی شامل ۱۹ سناریوی PostgreSQL، ۸۵ تست Web/Proxy، بررسی Contracts/Database، lint، typecheck و Production Build موفق‌اند. مرورگر: ۱۲ سکشن، CRUD متقاضی، ثبت اعلان، ویرایش کارمند، بایگانی رزومه، ورود اکسل معتبر/تکراری، PDF فارسی، مسیر قدیمی و عرض موبایل ۳۹۰ پیکسل تأیید شدند.
- دو Migration افزایشی HR با تحویل صریح قفل PC-A روی دیتابیس محلی اعمال شدند؛ تاریخچه Migrationهای خارجی حفظ شده است. اسکریپت اجرای لوکال نسخه و سلامت API را کنترل می‌کند. شاخه برای Review پوش شده و Merge انجام نشده است.
- وابستگی باز: تأیید/پرداخت مالی، محاسبه حقوق، اتصال دستگاه حضور و دریافت خودکار از سایت‌های کاریابی نیازمند قرارداد و Adapter ماژول مالک هستند؛ نتیجه ساختگی نمایش داده نمی‌شود. جزئیات Migration، آزمون و محدودیت‌ها: `docs/tasks/HR-005.md`.
