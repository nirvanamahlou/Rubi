...
شناسه ملی شرکت در فرم اولیه مشخص‌تر شده و مشخصات، شعب آژانس، نمایندگان، امضاداران و مدیر حساب در یک صفحه با فرم‌های پاپ‌آپی قرار گرفته‌اند. امضاداران مدل و API مستقل با شخص همان سازمان، حدود اختیار، ارز، بازه اعتبار و نسخه مدرک دارند؛ ثبت بدون مدرک غیرفعال است. ویرایش و حذف نسخه‌دار با Audit اتمیک انجام می‌شود. ۹۰ تست Web، ۴۹۶ تست API، ۱۶ تست PostgreSQL و ۶۱ تست Contracts موفق‌اند؛ lint/typecheck و build بسته‌های API/Database/Contracts پاس شده است. مهاجرت افزایشی روی نسخه بازیابی‌شده دیتابیس با حفظ داده‌های ۱۲۷ جدول آزموده شد. آماده Build نهایی Web و فعال‌سازی محلی؛ جزئیات و نتیجه نهایی: [B2B-UNIFIED-PROFILE-001](tasks/B2B-UNIFIED-PROFILE-001.md).
