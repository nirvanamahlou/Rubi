...
توسعه تکمیلی همین واحد، صفحه اصلی را به Hub دوکارتی «مدیریت قیمت» و «پک جنریتور» تبدیل کرد. صفحه فعلی قیمت‌گذاری در `/sales/pricing/management` حفظ شده و `/sales/pricing/generator` انتخاب پکیج/نوبت و پنل بومی الهام‌گرفته از Package Generator مرجع را با گالری قالب، گروه‌های ویرایش و Preview زنده ارائه می‌کند. Assetهای ۳۳۰ مگابایتی و Vendor ابزار مستقل کپی نشدند؛ داده همچنان واقعی، مجوزها deny-by-default و خروجی Documents غیرفعال است. ۲۱ تست ماژول، Prettier، lint، typecheck و build تولیدی ۵۲ مسیر موفق‌اند؛ Hub در دسکتاپ و موبایل مرورگر QA شد.
