...
## WORKBENCH-002 — تطبیق نمونه تعاملی میزکار — آماده بررسی

تغییرات موجود با اجازه صریح مالک تحویل گرفته شد. طراحی HTML مرجع با CSS اصلی حفظ شده و فرم‌ها و چرخه درخواست در محیط نمونه قابل استفاده‌اند. مسیر `/workbench/demo` فقط با `RUBI_WORKBENCH_DEMO=1` فعال است؛ پیش‌نمایش مستقل روی loopback پورت 3301 ارائه می‌شود. داده‌ها در حافظه‌اند و بازخوانی صفحه آن‌ها را بازنشانی می‌کند؛ اتصال عملیاتی و ذخیره پایدار ادعا نمی‌شود.

۹ تست میزکار، Web lint/typecheck و build موفق‌اند. چرخه ساخت تا بستن درخواست، یادداشت، پیام نمونه و چیدمان دسکتاپ/موبایل در مرورگر بررسی شد. بدون Migration، تغییر Dependency یا Merge. جزئیات تحویل و محدودیت‌ها: [WORKBENCH-002](tasks/WORKBENCH-002-DEMO-PARITY.md).
