...
مالک پروژه در 2026-09-16 مجوز صریح توسعهٔ Backendهای باقی‌ماندهٔ مدیریت سامانه را به `PC-B` داد. این اختیار در چهار Work Item مستقل Settings API، IAM Sessions، Operations Health و Governance اعمال می‌شود و شامل تغییر کد مالک، قرارداد عمومی، permission catalog و grant حداقلی، Schema/Migration افزایشی، Job/Worker، Seed تستی و تنظیمات غیرمحرمانه پس از رزرو دقیق فایل‌هاست. اجرای Migration و عملیات داده/نشست/Backup روی محیط ایزولهٔ local/test مجاز است؛ عملیات Production فقط با تأیید موردی اجرا، credential محیط و Audit مجاز می‌شود. هر حوزه Branch/PR مستقل دارد و Migration نیازمند `Migration Owner` اختصاصی و نبود lock هم‌پوشان است. دسترسی مستقیم میان جدول‌های مالک‌ها، افشای داده حساس، Grant گسترده، Push مستقیم به `main`/`develop` و دورزدن Review همچنان ممنوع است. PR #304 در محدودهٔ فعلی UI-only باقی می‌ماند.
