# وضعیت پروژه
## B2B-LIVE-KPIS-001 — اتصال شاخص‌ها به داده

شمارش آژانس و مشتری سازمانی از API عمومی اطلاعات پایه با جست‌وجو/وضعیت و پوشش همه صفحات خوانده می‌شود. کارت تکمیل هویت، نبود نوع شخصیت یا شناسه ملی شخصیت حقوقی را بدون شمارش تکراری نشان می‌دهد؛ خطا صفر نیست. ذخیره/حذف/ورود اکسل آمار را تازه می‌کند. ۱۰۳ تست و lint/typecheck موفق؛ PR163 پیش‌نویس و source c63ec00+8b6cd35 برای build مشترک۳۱۰۰ تحویل مالک میزکار شد. بدون تغییر API/داده.
...
## B2B-FORM-SAVE-AUDIT-001 — PC-B — 2026-09-11
Authenticated agency UI save audit on3100 isolated all test writes in synthetic organization ORG_Q9WGXUJ2F2AN. Fixed missing representative preferredChannel in cooperation wizard and calendar expiry display in organization document cards.107 Organizations tests, Web lint and TypeScript pass. Combined production build includes wizard fix; final expiry display build/retest coordinated with native workbench owner. See B2B_FORM_SAVE_AUDIT_2026-09-11.md for observed saves and explicit limitations. Draft PR168; no migration, dependency, API or existing-record changes. New-user credential entry requires user handoff; no new login account created.
Final runtime004c669 / hr005-02787b347c596a3e: combined build and live expiry display retest passed on3100. All six financial document upload forms saved; logo and organization/contact/address edits persisted. Draft PR168; source locks released. Only new-user credential submission remains with user; prepared inactive form, password blank. No complete-all-forms claim.
## B2B-PROFILE-GRID-001 — PC-B
Aligned organization dossier card heights by stretching each grid row and nested card wrappers. Scope is corporate-profile.tsx and corporate-design.css; no fixed heights, data, API or dependency changes.107 Organizations tests, scoped ESLint and TypeScript pass. Combined runtime integration uses004c669 +5ef4e39 in isolated Rubi-profile-grid-runtime; Web3100 temporarily delegated by native workbench owner, API4190 preserved. Browser baseline reproduced247px and140px empty areas beneath short cards; final post-build checks pending. Draft PR170.
B2B-PROFILE-GRID-001: source5ef4e39+a10b7bd; nested card panels now stretch and local panel bottom margin is zero, leaving only14px grid gap. Browser390px check: all five cards one column343px wide, document width375px equals scrollWidth. Web ownership returned to native workbench owner for final combined build including both commits.
Final verification on combined828af98 / hr005-8034d7d7d02efa2b at3100: first-row cards both608.6875px, second-row cards both436.71875px, every card gapBelow0, inter-row gap14px. Desktop screenshot and mobile single-column checks passed.107 tests/lint/typecheck and combined build passed. Source locks released; PR170 remains draft, no merge.
...
Runtime QA: real PDF (2 pages) and XLSX downloaded for agreements; separate XLSX downloads verified for rates/discounts/commission/credit/guarantees. Mobile390px has no overflow. Visual PDF review found date bidi issue, corrected7fc5b05 with8 focused tests. Combined final1af4c0c/hr005-b7ab66ff79101b7b/PID19708 served by workbench owner,43-route build and typecheck pass; API unchanged. Draft PR189.
Final1af4c0c PDF re-download and Poppler rendering verified: Persian shaping clear and ISO date2026-09-11 preserves LTR order. No clipping. Source/worktree clean after docs; Web remains with workbench owner.
