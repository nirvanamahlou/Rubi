...
## SALES-001-MOCKUP — قراردادهای من و تکمیل مرحله‌ای — آماده بررسی

- `PC-A` روی شاخه مستقل `codex/pc-a-sales-contracts-mockup`، مسیر `/sales` را از Foundation عمومی به ماکاپ تخصصی فارسی/RTL «قراردادهای من» تبدیل کرد. بالای صفحه دکمه «قرارداد جدید» و برای هر قرارداد مرحله فعلی، درصد پیشرفت، مسافر/خدمت، وضعیت مالی، کارشناس و اقدام بعدی نمایش داده می‌شود.
- Wizard پنج‌مرحله‌ای اطلاعات قرارداد، مسافران، تخصیص خدمات، قیمت/شرایط و تأیید نهایی را پوشش می‌دهد. جست‌وجو، فیلتر وضعیت، Empty state و جزئیات قرارداد قابل تعامل‌اند؛ همه داده‌ها ساختگی و بدون Persistence/API/Migration هستند.
- با درخواست تکمیلی مالک، فایل مستقل `mockups/sales-contracts.html` نیز تحویل شد؛ برای مشاهده به Server، Port، Login یا Build نیاز ندارد و همه استایل‌ها و تعاملات داخل همان فایل هستند.
- Build تولیدی Web، lint کامل، typecheck و ۶ تست هدفمند موفق‌اند؛ Route تولیدی `/sales` ساخته شد. پیش‌نمایش محلی روی پورت مستقل ۳۱۱۲ فعال است. انتشار عمومی انجام نشد چون Hosting سازگار با Sites در این Repository تعریف نشده و افزودن آن خارج از Scope است.
