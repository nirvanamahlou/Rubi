...
تقویم‌های فرم‌های پرونده در پنجره‌ها اصلاح، انواع قرارداد تکمیل و روش پرداخت به مرجع واقعی اطلاعات پایه متصل شد. بارگذاری سند قرارداد و تضمین در فرم همان مورد انجام می‌شود؛ بخش مستقل بارگذاری و کنترل نوع سقف از UI حذف شده‌اند. فهرست نرخ توافقی پس از انتخاب بسته می‌شود. آزمون‌های هدفمند، PostgreSQL و بارگذاری واقعی دو سند ساختگی CLEAN موفق‌اند. Migration افزایشی با backup/rehearsal و حفظ داده‌های ۱۲۹ جدول اعمال شده؛ build و cutover نهایی ۳۱۰۰ در حال انجام است. جزئیات: [B2B-CONTRACT-FORMS-002](tasks/B2B-CONTRACT-FORMS-002.md).
