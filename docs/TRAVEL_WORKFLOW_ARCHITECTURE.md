...
رزرواسیون حق ایجاد یا تغییر رابطه مشتری/مسافر با قرارداد و خدمات را ندارد. چیدمان اجرایی هتل برای همان مسافران Snapshot می‌تواند با نسخه جدید و Audit در رزرواسیون اصلاح شود؛ مسافر جدید یا تغییر صندلی نیازمند اصلاح Sales است. اگر
...
- نسخه قیمت خرید مرجع، ارز و اعتبار آن. طبق تصمیم جدید `SALES-TICKET-PRICES-0922` نرخ عمومی فروش یک‌طرفه و نرخ عمومی جفت رفت‌وبرگشت به‌صورت versioned در Ticket Catalog نگه‌داری و از زیرماژول فروش مدیریت می‌شوند؛ تخفیف، markup، commission و توافق مشتری همچنان فقط در snapshot قرارداد Sales هستند.
...
استثنای محدود تأییدشده برای بلیت پیش‌فروش تور: ثبت آفر واقعی، یک درخواست خرید
بدون مبلغ با FK همان آفر/نسخه در Procurement می‌سازد. پیش‌نویس محلی بلیت
درخواست مالی موازی ایجاد نمی‌کند. مالی قیمت خرید بزرگسال/کودک و فاکتور را ثبت
و پرداخت را مستقیم در revisionهای مالی ثبت می‌کند؛ تأیید جداگانه خرید/تأمین
برای این مسیر لازم نیست. پرداخت جزئی قیمت را برای فروش آزاد نمی‌کند؛ تسویه کامل
به‌صورت اتمی درخواست را PAID کرده و cost را از قرارداد عمومی مالی به قیمت‌گذاری
تور می‌دهد. این استثنا approval خریدهای پس از قرارداد یا قواعد release مدارک
مسافر را تغییر نمی‌دهد. ثبت payment evidence به معنی اجرای بانکی/journal نیست.
...
3. هر اصلاح تاریخ اقامت، تعداد/نوع اتاق و رده سنی یک نسخه جدید از فرم رزرواسیون
   می‌سازد. انتخاب دامنه اصلاح مشخص می‌کند قرارداد و واچر هم آن نسخه را مصرف کنند یا
   فقط فرم کارگزار و خرید؛ سابقه قبلی حذف نمی‌شود.
4. رزرواسیون درخواست خرید را با قیمت و تخفیف کارگزار و بر مبنای آخرین فرم واقعاً
   ارسال‌شده ثبت می‌کند.
5. تایید کارگزار و Confirmation Number ثبت می‌شود.
6. رزرواسیون واچر versioned را صادر می‌کند.
7. مالی مجوز تحویل می‌دهد و فروش واچر را برای مسافر ارسال می‌کند.
