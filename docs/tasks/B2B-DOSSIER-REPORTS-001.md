...
- Reports/Audit/Export now load the actual persisted B2B, Master Data and Documents audit streams. Summary counts, chronological rows, actor names, outcome, changed-field labels and details are available. Branch, category, source, outcome and inclusive Tehran-day date filters work. The existing Persian date picker and Nora typography are reused.
