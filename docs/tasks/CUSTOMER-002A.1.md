...
- کد ملی مشتری و هر مسافر باید در Create/Import آینده اجباری، ده‌رقمی و دارای checksum معتبر باشد؛ Schema و قرارداد فعلی Customers چنین فیلدی ندارند. در CUSTOMER-002B ذخیره امن، fingerprint/unique policy، masking، permission/audit و Migration آن باید یکپارچه پیاده‌سازی شود و تا آن زمان فیلد صوری یا بدون Persistence نمایش داده نمی‌شود.
...
- خروجی همه رکوردهای فیلترشده (نه فقط صفحه جاری) با فرمت واقعی XLSX و شماره تماس ماسک‌شده؛ ورود گروهی XLSX با قالب دانلودی که فقط ستون «نام مشتری» را اجباری می‌کند.
- فهرست مشتریان و مسافران با ۲۰ نفر در هر صفحه، شماره همراه ماسک‌شده و نمایش «صفحه X از Y» ارائه می‌شود.
- در فرم ایجاد، فقط ردیف مسافر شماره ۱ می‌تواند از اطلاعات همان مشتری بالای فرم استفاده کند؛ فیلدهای مشتری به‌صورت خودکار در این ردیف نمایش داده می‌شوند و همان رکورد بدون ایجاد Duplicate نقش مسافر می‌گیرد. همراه‌های بعدی همچنان مسافر مستقل هستند.
