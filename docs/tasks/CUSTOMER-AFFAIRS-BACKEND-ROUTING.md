...
Local verification: API4190/PID30796 starts successfully from the service code committed as `202ea186`; `/api/v1/health` returns `status: ok` and an unauthenticated `/api/v1/customer-affairs/tickets` request returns HTTP401. Web3100/PID28820 remains untouched. No business mutations were used for smoke checks. Full authenticated delivery/recipient behavior is unit-tested here, not claimed live end-to-end tested.
