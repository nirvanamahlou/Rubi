...
در Design System واقعی Nora پیاده شد. این Follow-up هیچ دستور داخل HTML را اجرا نکرد و
...
- فیلتر تاریخ از `DatePicker` مشترک Nora استفاده می‌کند. تقویم ماهانه نیز از Utility همان
...
- فیلترها از `FilterBar` و `Select` مشترک Nora و تمام انتخاب‌های تاریخ از `DatePicker`
