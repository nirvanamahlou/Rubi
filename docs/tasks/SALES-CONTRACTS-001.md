...
## Passenger capacity and hotel members — 2026-09-06

- The first step now captures adult, child and infant counts before inventory selection. Adults and children consume seats; infants remain part of the traveller total but do not reduce ticket capacity. The actual passenger records must match the configured age composition before the contract can continue.
- Ticket search exposes producer-owned **remainingCapacity**. The UI disables offers below the required seated count. Confirmation atomically locks the selected offer rows and creates idempotent allocations by opaque Sales contract ID and direction; cancellation releases active allocations. Sales only calls the Ticket Catalog public service and does not query its table.
- Hotel occupancy is derived from explicit guest membership selected from the contract passengers. Passenger service allocations carry that membership through the existing Sales contract and immutable Reservations snapshot. Hotel availability remains `NEEDS_RESERVATION_CONFIRMATION`.
- Added migration `20260906095000_ticket_offer_capacity_allocations`. Prisma format/generate/validate, all 33 migrations on fresh PostgreSQL 18, Web/API lint/typecheck/build, 678 Web tests and 823 API tests passed (75 optional API tests skipped); 46 focused Web tests and 31 focused API tests and the dedicated PostgreSQL oversell race test passed. The pre-existing concurrent Reservations upsert case failed once in the full PostgreSQL file with its own unique-key race; the new capacity test passed both in the full run and independently.