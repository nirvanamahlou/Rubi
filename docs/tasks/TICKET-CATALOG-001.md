...
At the product owner's request, a paused catalog ticket now exposes a visible «فعال‌کردن فروش» power action, while active tickets retain «توقف فروش». Draft and cancelled tickets do not expose a lifecycle power action. Every catalog ticket can be opened for editing regardless of lifecycle status; editing preserves that status. The existing optimistic-version, reference, capacity and allocated-inventory guards remain enforced, and cancelled tickets still cannot transition back to active.
