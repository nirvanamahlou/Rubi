param(
  ...
  [ValidateRange(1,65535)][int]$ApiPort = 4000,
  [string]$ApiEnvFile,
  [string]$DatabaseName,
...
if (!$SkipApi) {
  ...
  $apiEnvironment = if ($ApiEnvFile) { [IO.Path]::GetFullPath($ApiEnvFile) } else { Join-Path $apiPath '.env' }
  if (!(Test-Path -LiteralPath $apiMain) -or !(Test-Path -LiteralPath $apiEnvironment)) { throw 'API build or local .env is missing.' }
  if ($Port -eq $ApiPort) { throw 'Web and API ports must differ.' }
  $apiLauncher = Join-Path $PSScriptRoot 'start-hr-api.mjs'
  $apiArguments = @(('--env-file="' + $apiEnvironment + '"'),('"' + $apiLauncher + '"'),'--api-port',"$ApiPort",'--web-port',"$Port")
  if ($DatabaseName) { $apiArguments += @('--database',$DatabaseName) }
  if ($DocumentStorageRoot) { $apiArguments += @('--documents',('"' + [IO.Path]::GetFullPath($DocumentStorageRoot) + '"')) }
  Stop-OwnedListener $ApiPort
  $apiProcess = Start-Process -FilePath $nodePath -ArgumentList $apiArguments -WorkingDirectory $apiPath -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $runtimePath 'api.log') -RedirectStandardError (Join-Path $runtimePath 'api-error.log')
}
$ready = $false
...
if (!$SkipApi) {
  $apiReady=$false
  for($attempt=0;$attempt -lt 30;$attempt++) {
    try { $health = Invoke-WebRequest -Uri "http://localhost:$ApiPort/api/v1/health" -TimeoutSec 3 -UseBasicParsing; if($health.StatusCode -eq 200){$apiReady=$true;break} } catch { Start-Sleep -Milliseconds 500 }
  }
  if(!$apiReady){throw 'HR API did not become healthy; inspect tmp/hr-runtime/api-error.log.'}
}
@{url="http://localhost:$Port/hr";version=$served.version;commit=$served.commit;webPid=$webProcess.Id;apiPort=$ApiPort;apiHealthy=(!$SkipApi -and $apiReady)} | ConvertTo-Json
