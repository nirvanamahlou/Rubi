...
describe('legal-entities.v1 public contract', () => {
  it('publishes only the two real issuer records and keeps ALL virtual', () => {
    expect(LEGAL_ENTITIES_CONTRACT_VERSION).toBe(1);
  ...
  it('publishes deny-by-default legal entity permissions', () => {
    ...
    expect(LEGAL_ENTITY_PERMISSION_CODES).toContain('legal-entity.aggregate.read');
    expect(LEGAL_ENTITY_PERMISSION_CODES).toContain('legal-entity.document.reissue');
  });
});
