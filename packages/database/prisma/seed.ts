    ...
    await database.$transaction(
      async (transaction) => {
        ...
        const documentRoleSpecs = [
          ...
          {
            code: 'customer_affairs_agent',
            name: 'کارشناس امور مشتریان',
            permissions: [
              'customers.read',
              'documents.list',
              'documents.metadata.read',
          ...
          {
            code: 'customer_affairs_supervisor',
            name: 'سرپرست امور مشتریان',
            permissions: [
              'customers.read',
              'documents.list',
              'documents.metadata.read',
        ...
        await Promise.all(
          seededPermissions.filter((permission) => !permission.code.startsWith('procurement.')).map((permission) =>
