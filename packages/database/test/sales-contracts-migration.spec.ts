...
const migration = readFileSync(
  resolve(
    process.cwd(),
    'prisma/migrations/20260903123000_sales_contracts_vertical_slice/migration.sql',
  ),
  'utf8',
);
...
const seed = readFileSync(
  resolve(process.cwd(), 'src/permission-seed-data.ts'),
  'utf8',
);
...
describe('SALES-CONTRACTS-001 migration', () => {
  it('creates the additive sales boundary with internal foreign keys', () => {
    for (const table of [
      'sales_contracts',
      'sales_contract_passengers',
      'sales_contract_services',
      'sales_contract_ticket_selections',
      'sales_contract_hotel_selections',
      'sales_contract_price_components',
      'sales_contract_payment_entries',
      'sales_reservation_requests',
      'sales_contract_status_history',
      'sales_contract_audit_events',
    ])
